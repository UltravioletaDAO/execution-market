...
def fetch_all_tasks(client: EMApiClient, status: str = "completed") -> list[dict]:
    ...
    while True:
        result = client._request("GET", f"/api/v1/tasks?status={status}&limit={limit}&offset={offset}")
...
class ProductionAnalytics:
    ...
    def _compute(self):
        ...
        for task in self.tasks:
            ...
            if created:
                if chain not in self.chain_first_seen or created < self.chain_first_seen[chain]:
                    self.chain_first_seen[chain] = created
                if chain not in self.chain_last_seen or created > self.chain_last_seen[chain]:
    ...
    def chain_report(self) -> dict:
        ...
        for chain in sorted(self.chain_counts, key=self.chain_counts.get, reverse=True):
            report[chain] = {
                ...
                "avg_bounty_usd": round(self.chain_bounties[chain] / self.chain_counts[chain], 2) if self.chain_counts[chain] else 0,
    ...
    def time_report(self) -> dict:
        ...
        # Find busiest day
        busiest_day = self.tasks_by_day.most_common(1)[0] if self.tasks_by_day else ("none", 0)
        
        # Find busiest hour
        busiest_hour = self.tasks_by_hour.most_common(1)[0] if self.tasks_by_hour else (0, 0)
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "avg_bounty_usd": round(self.total_bounty / len(self.tasks), 2) if self.tasks else 0,
...
class ProductionProfileBuilder:
    ...
    def _build_executor_profile(self, executor_id: str, tasks: list[dict]) -> dict:
        ...
        for task in tasks:
            ...
            # Estimate completion time from created_at → status
            created = task.get("created_at", "")
            ...
            if created and deadline:
                try:
                    ...
                    completion_times.append(delta_hours)
                except (ValueError, AttributeError):
                    pass
        ...
        max_category_share = max(categories.values()) / total_tasks if total_tasks else 0
        chain_diversity = len(chains) / 8  # Normalized against 8 possible chains
        
        return {
            "executor_id": executor_id,
            "total_tasks": total_tasks,
            "total_earned_usd": round(total_bounty, 2),
            "avg_bounty_usd": round(total_bounty / total_tasks, 2) if total_tasks else 0,
...
def cmd_full(args):
    ...
    for chain, data in chain_report.items():
        ...
        color = C.GREEN if data["tasks"] >= 10 else C.YELLOW if data["tasks"] >= 5 else C.DIM
        print(f"  {color}{chain:12s}{C.RESET} {bar} {data['tasks']:3d} tasks (${data['total_bounty_usd']:.2f})")
    ...
    if time_report:
        ...
        metric("Busiest day", f"{time_report['busiest_day']['date']} ({time_report['busiest_day']['count']} tasks)")
    ...
    for executor_id, profile in profiles.items():
        ...
        print(f"  {C.GREEN}●{C.RESET} {executor_id[:12]}... — {profile['total_tasks']} tasks, "
              f"${profile['total_earned_usd']:.2f}, {exp}")
        print(f"    {C.DIM}Chains: {chains_str} | Primary: {profile['skill_dna']['primary_category']}{C.RESET}")
    ...
    if expired:
        ...
        metric("Expiry rate", f"{expiry_rate:.1f}%", C.YELLOW if expiry_rate > 30 else C.GREEN)
    ...
    if len(analytics.category_counts) == 1:
        warn(f"All {len(completed)} tasks are '{list(analytics.category_counts.keys())[0]}' — "
    ...
    if worker_concentration > 90:
        warn(f"Worker concentration: {worker_concentration:.0f}% tasks by single executor — "
             f"need more workers for swarm routing diversity")
    
    # Insight 4: Expiry rate
    if expired and len(expired) > len(completed) * 0.3:
        warn(f"High expiry rate ({len(expired)}/{len(completed) + len(expired)} = "
...
def cmd_summary(args):
    ...
    metric("Chains active", f"{summary['unique_chains']} ({', '.join(summary['chains'].keys())})")
    metric("Tokens used", f"{summary['unique_tokens']} ({', '.join(summary['tokens'].keys())})")
    metric("Unique executors", summary["unique_executors"])
    print()
...
def cmd_chains(args):
    ...
    for chain, data in chain_report.items():
        ...
        print(f"  {chain:12s} {data['tasks']:6d} {data['share_pct']:6.1f}% "
...
def main():
    ...
    parser.add_argument("--chains", action="store_true", help="Chain utilization analysis")
