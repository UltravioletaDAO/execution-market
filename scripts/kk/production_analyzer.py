...
class AnalysisReport:
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            f"# Execution Market Production Analysis",
            f"",
        ...
        for etype, count in sorted(self.evidence_type_distribution.items(), key=lambda x: -x[1]):
        ...
        lines.extend([
            f"",
            f"---",
            f"*Generated by KK V2 Production Analyzer*",
        ])
...
class ProductionAnalyzer:
    ...
    def analyze(self, verbose: bool = True) -> AnalysisReport:
        ...
        all_tasks = []
        valid_statuses = [
            "completed", "submitted", "published", "accepted",
            "in_progress", "verifying", "expired", "cancelled", "disputed",
        ]
        ...
        if verbose:
            ...
            metric("Category diversity", f"{report.category_diversity_score:.3f}",
                   C.GREEN if report.category_diversity_score > 0.5 else C.YELLOW)
            
            # Show category distribution
            print(f"\n  {C.BOLD}Category Distribution:{C.RESET}")
            max_cat_count = max(report.tasks_by_category.values()) if report.tasks_by_category else 1
            for cat, count in sorted(report.tasks_by_category.items(), key=lambda x: -x[1])[:10]:
                pct = count / report.total_tasks * 100
                print(f"    {cat:25s} {bar(count, max_cat_count, 25)} {count:4d} ({pct:.0f}%)")
        ...
        if verbose:
            ...
            for i, wp in enumerate(report.worker_profiles[:5], 1):
                ...
                print(f"    {i}. {C.CYAN}{wp.wallet[:10]}...{C.RESET} — "
        ...
        if verbose:
            ...
            if report.evidence_type_distribution:
                ...
                max_ev = max(report.evidence_type_distribution.values()) if report.evidence_type_distribution else 1
                for etype, count in sorted(report.evidence_type_distribution.items(), key=lambda x: -x[1]):
                    print(f"    {etype:20s} {bar(count, max_ev, 20, C.BLUE)} {count:4d}")
        ...
        if verbose:
            ...
            metric("Tasks unassigned", sim.get("tasks_unassigned", 0),
    ...
    def _analyze_tasks(self, tasks: list, report: AnalysisReport):
        ...
        if category_counts:
            ...
            report.category_diversity_score = entropy / max_entropy if max_entropy > 0 else 0.0
    ...
    def _analyze_workers(self, tasks: list, report: AnalysisReport):
        ...
        for wallet, data in worker_data.items():
            ...
            hhi = sum((c / total) ** 2 for c in data["categories"].values()) if total > 0 else 0.0
        ...
        report.avg_tasks_per_worker = (
            sum(p.tasks_completed for p in profiles) / len(profiles) if profiles else 0.0
        )
        report.specialization_index = (
            sum(p.specialization_score for p in profiles) / len(profiles) if profiles else 0.0
        )
    ...
    def _assess_swarm_readiness(self, report: AnalysisReport):
        ...
        if completed >= 100:
            checks["task_volume"] = {"pass": True, "message": f"{completed} completed tasks — strong signal"}
        elif completed >= 20:
            checks["task_volume"] = {"pass": True, "warn": True, "message": f"{completed} completed — growing"}
        else:
            checks["task_volume"] = {"pass": False, "message": f"Only {completed} completed — need more data"}
        ...
        if div > 0.5 and num_cats >= 3:
            checks["category_diversity"] = {"pass": True, "message": f"Diversity {div:.2f} across {num_cats} categories"}
        elif num_cats >= 2:
            checks["category_diversity"] = {"pass": True, "warn": True, "message": f"Low diversity ({div:.2f}) — {num_cats} categories"}
        else:
            checks["category_diversity"] = {"pass": False, "message": f"Only {num_cats} category — need diversity for meaningful routing"}
        
        # Check 3: Worker pool
        if report.total_workers >= 10:
            checks["worker_pool"] = {"pass": True, "message": f"{report.total_workers} workers — healthy pool"}
        elif report.total_workers >= 3:
            checks["worker_pool"] = {"pass": True, "warn": True, "message": f"{report.total_workers} workers — growing"}
        else:
            checks["worker_pool"] = {"pass": False, "message": f"Only {report.total_workers} workers — need more"}
        
        # Check 4: Evidence quality
        if report.avg_evidence_per_task >= 1.5:
            checks["evidence_quality"] = {"pass": True, "message": f"Avg {report.avg_evidence_per_task:.1f} evidence items/task"}
        elif report.avg_evidence_per_task >= 0.5:
            checks["evidence_quality"] = {"pass": True, "warn": True, "message": f"Low evidence density ({report.avg_evidence_per_task:.1f}/task)"}
        else:
            checks["evidence_quality"] = {"pass": False, "message": "Insufficient evidence data"}
        ...
        try:
            ...
            checks["swarm_modules"] = {"pass": True, "message": "All modules importable"}
...
def main():
    ...
    parser.add_argument("--export", choices=["json", "md", "both"], help="Export format")
    parser.add_argument("--quiet", "-q", action="store_true", help="Suppress terminal output")
    parser.add_argument("--em-url", default="https://api.execution.market", help="EM API URL")
    ...
    if not args.quiet:
        ...
        color = C.GREEN if passed == total_checks else C.YELLOW if passed > total_checks / 2 else C.RED
        print(f"  {color}{passed}/{total_checks} readiness checks passed{C.RESET}")
        print(f"  {report.total_tasks} tasks | {report.total_workers} workers | "
