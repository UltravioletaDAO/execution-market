...
from swarm.reputation_bridge import ReputationBridge, ReputationTier
from swarm.lifecycle_manager import LifecycleManager, AgentState, BudgetConfig
from swarm.orchestrator import SwarmOrchestrator, TaskRequest, TaskPriority, RoutingStrategy
from swarm.autojob_client import AutoJobClient, EnrichedOrchestrator
from swarm.event_listener import EventListener, ListenerState
from swarm.evidence_parser import EvidenceParser, WorkerRegistry, SkillDimension
...
def cmd_status(args):
    ...
    ok("ReputationBridge — on-chain + internal scoring")
    ok("LifecycleManager — 7-state machine + budget")
    ok("SwarmOrchestrator — 4 routing strategies")
    ok("AutoJobClient — enrichment bridge")
    ok("SwarmCoordinator — operational controller")
    ok("EventListener — API polling + watermarks")
    ok("EvidenceParser — Skill DNA extraction")
...
def cmd_dashboard(args):
    ...
    metric("Tasks failed", tasks.get("failed", 0), C.RED if tasks.get("failed") else C.GREEN)
    ...
    metric("Agents degraded", agents.get("degraded", 0), C.YELLOW if agents.get("degraded") else C.GREEN)
    metric("Agents suspended", agents.get("suspended", 0), C.RED if agents.get("suspended") else C.GREEN)
    ...
    for agent in fleet[:10]:  # Show top 10
        ...
        print(f"  Agent {agent_id}: {state_color}{state}{C.RESET} | Budget: ${remaining:.2f}")
...
def cmd_agents(args):
    ...
    for agent_id in range(2101, 2125):
        ...
        if identity and not identity.get("error"):
            name = identity.get("name", identity.get("display_name", f"Agent-{agent_id}"))
...
def cmd_poll(args):
    ...
    metric("Completions", result.completed_tasks, C.GREEN if result.completed_tasks else C.DIM)
...
def cmd_health(args):
    ...
    if os.path.exists(state_path):
        ...
        ok(f"Listener state: {state.poll_count} polls, {state.total_new_tasks} tasks seen")
        checks_passed += 1
    else:
...
def cmd_simulate(args):
    ...
    subheader("Ingesting Tasks")
    tasks = [
        ("Deliver package to 5th Ave", ["delivery", "physical"], 15.0, TaskPriority.NORMAL),
        ...
        ("Research competitor pricing", ["research", "analysis"], 20.0, TaskPriority.NORMAL),
        ...
        ("Geo-verify construction site", ["verification", "geo"], 10.0, TaskPriority.NORMAL),
    ]
    ...
    for task_id, task in coordinator._task_queue.items():
        if task.assigned_agent_id:
            ok(f"{task.title[:40]:40s} → Agent {task.assigned_agent_id} (${task.bounty_usd:.0f})")
        else:
    ...
    completions = [
        ("sim-001", 2101, [
            {"type": "photo_geo", "content": "delivery-proof.jpg", "metadata": {"latitude": 25.76}},
            {"type": "receipt", "content": "Receipt #456"},
        ]),
        ("sim-005", 2102, [
            {"type": "screenshot", "content": "api-test-output.png"},
            {"type": "text_response", "content": "Implemented REST API with 15 endpoints, full test coverage, and documentation. " * 3},
            {"type": "document", "content": "API Documentation v1.0"},
        ]),
        ("sim-002", 2105, [
            {"type": "document", "content": "Smart contract audit report covering 12 contracts. " * 5},
            {"type": "text_response", "content": "No critical vulnerabilities found. 3 medium-severity issues documented."},
        ]),
    ]
    ...
    for task_id, agent_id, evidence in completions:
        ...
        ok(f"Task {task_id}: quality={assessment.quality.value} score={assessment.score:.2f}")
        info(f"  → Agent {agent_id} Skill DNA: {[f'{k}={v:.2f}' for k,v in dna.get_top_skills(3)]}")
...
def cmd_benchmark(args):
    ...
    ok(f"{n_tasks} tasks ingested in {ingest_ms:.1f}ms ({ingest_ms/n_tasks:.2f}ms/task)")
    ...
    ok(f"{n_parse} evidence sets parsed in {parse_ms:.1f}ms ({parse_ms/n_parse:.3f}ms/set)")
    ...
    ok(f"{n_updates} DNA updates in {update_ms:.1f}ms ({update_ms/n_updates:.3f}ms/update)")
...
Commands:
  ...
  benchmark  Performance benchmark
    ...
    parser.add_argument("command", choices=[
        "status", "dashboard", "agents", "poll", "health", "simulate", "benchmark",
    ])
