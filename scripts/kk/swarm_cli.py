...
def cmd_tasks(args):
    ...
    total_bounty = sum(
        float(t.get("bounty_usd", t.get("bounty_amount", 0))) for t in all_tasks
    )
...
def cmd_status(args):
    ...
    try:
        ...
        test_files = list(
            (project_root / "mcp_server" / "tests").glob("test_swarm*.py")
        )
...
Examples:
  ...
  %(prog)s history [task_id]    Task history/details
    ...
    tasks_parser.add_argument(
        "--limit", type=int, default=20, help="Max tasks per status"
    )
    ...
    sim_parser.add_argument(
        "--agents", type=int, default=10, help="Number of simulated agents"
    )
    sim_parser.add_argument(
        "--budget", type=float, default=5.0, help="Agent daily budget"
    )
    ...
    run_parser.add_argument(
        "--interval", type=float, default=120, help="Cycle interval in seconds"
    )
    run_parser.add_argument(
        "--max-tasks", type=int, default=10, help="Max tasks per cycle"
    )
    run_parser.add_argument(
