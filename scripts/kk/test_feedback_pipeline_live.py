...
from swarm.feedback_pipeline import FeedbackPipeline
...
def main():
    ...
    if not completed:
        ...
        for status in ["approved", "in_review"]:
            result = pipeline._em_request("GET", f"/api/v1/tasks?status={status}&limit=50")
            if result:
    ...
    for task in tasks_to_process:
        ...
        if feedback.error:
            ...
            quality_counts[feedback.quality.value] = quality_counts.get(feedback.quality.value, 0) + 1
            print(
    ...
    for worker_id in list(workers_seen)[:5]:
        ...
        if profile:
            ...
            if top_skills:
                skills_str = ", ".join(f"{s['skill']}={s['score']:.2f}" for s in top_skills[:3])
            ...
            if composite:
                print(f"     Composite: {composite.get('total', 0):.1f} (tier={composite.get('tier', '?')})")
