#!/usr/bin/env python3
"""
Fase 3 Clean Escrow E2E Test — No on-chain operator fee on Base Mainnet.

Tests the clean Fase 3 operator: OR(Payer|Facilitator) conditions, feeCalculator=address(0).
No operator fee is deducted. Only x402r ProtocolFeeConfig (currently 0%) can take on-chain fees.

  1. AUTHORIZE  — Lock USDC in escrow via clean operator
  2. QUERY      — Verify escrow state (capturable > 0)
  3. RELEASE    — Release to worker (gasless via facilitator) — 100% to receiver
  -- OR --
  3b. REFUND   — Refund to agent (gasless via facilitator) — 100% back

Usage:
  python scripts/test-fase3-clean-escrow.py                    # Full lifecycle (authorize + release)
  python scripts/test-fase3-clean-escrow.py --refund           # Authorize + refund instead of release
  python scripts/test-fase3-clean-escrow.py --dry-run          # Print config, don't transact

Requires:
  - .env.local with WALLET_PRIVATE_KEY (dev wallet with USDC on Base)
  - uvd-x402-sdk >= 0.13.0
  - Facilitator v1.33.4+ with clean PaymentOperator registered
"""
...
# Fase 3 Clean PaymentOperator: OR(Payer|Facilitator), feeCalculator=address(0) — no operator fee
EM_OPERATOR = "0xd5149049e7c212ce5436a9581b4307EB9595df95"
FACILITATOR_URL = "https://facilitator.ultravioletadao.xyz"
RPC_URL = os.environ.get("BASE_RPC_URL", "https://mainnet.base.org")
CHAIN_ID = 8453

# Test amount: $0.002 USDC = 2000 atomic units (6 decimals)
...
TEST_RECEIVER = "YOUR_TEST_WORKER_WALLET"  # Test Worker
...
def main():
    ...
    print("=" * 60)
    print("Fase 3 Clean Escrow E2E Test — No Operator Fee — Base Mainnet")
    print("=" * 60)
    ...
    print()
    print("=" * 60)
    print("FASE 3 E2E TEST COMPLETE")
    print("=" * 60)
    print(f"Mode:           {'REFUND' if do_refund else 'RELEASE'}")
