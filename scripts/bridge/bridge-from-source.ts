...
async function bridgeToChain(
  ...
  if (route.provider === "debridge") {
    ...
    // Add 15% buffer — deBridge execution fee varies significantly between quote and TX
    const rawApprove = BigInt(
      quote.estimation?.srcChainTokenIn?.amount || atomicAmount,
    );
    const approveAmount = rawApprove + (rawApprove * 15n) / 100n;
    const spender = getAddress(quote.tx.to) as Address;
    ...
    if (currentAllowance >= approveAmount) {
      console.log(
        "    Allowance sufficient (" + formatUnits(currentAllowance, 6) + " >= " + formatUnits(approveAmount, 6) + "), skipping approve",
      );
    } else {
      console.log(
        "    Approving " + formatUnits(approveAmount, 6) + " USDC to " + spender + "...",
      );
