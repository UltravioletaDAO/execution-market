...
Tests the Fase 3 operator with OR(Payer|Facilitator) conditions + 1% on-chain fee:
  1. AUTHORIZE  — Lock USDC in escrow via Fase 3 operator
...
Usage:
  python scripts/test-fase3-escrow.py                    # Full lifecycle (authorize + release)
  python scripts/test-fase3-escrow.py --refund           # Authorize + refund instead of release
  python scripts/test-fase3-escrow.py --dry-run          # Print config, don't transact

Requires:
  - .env.local with WALLET_PRIVATE_KEY (dev wallet with USDC on Base)
  - uvd-x402-sdk >= 0.13.0
  - Facilitator v1.33.3+ with Fase 3 PaymentOperator registered
"""
...
# Fase 3 PaymentOperator: OR(Payer|Facilitator) release/refund + StaticFeeCalculator(100bps)
EM_OPERATOR = "0x8D3DeCBAe68F6BA6f8104B60De1a42cE1869c2E6"
...
def main():
    ...
    print("=" * 60)
    print("Fase 3 Escrow E2E Test — Trustless — Base Mainnet")
    print("=" * 60)
