...
export function ProfileEditModal({ executor, onClose, onSaved }: ProfileEditModalProps) {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="lg"
      labelledBy="profile-edit-modal-title"
      dismissOnEsc={!saving}
      dismissOnBackdrop={!saving}
    >
      <Modal.Header onClose={saving ? undefined : onClose}>
        <h2 id="profile-edit-modal-title" className="text-xl font-bold text-zinc-900">
      ...
      <Modal.Body className="space-y-5">
        ...
        <div className="flex items-center gap-4">
          <div
            className="relative w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center text-white text-xl font-bold cursor-pointer overflow-hidden group"
          ...
          <div>
            <button
              ...
              className="text-sm text-zinc-900 hover:text-zinc-700 font-medium hover:underline"
            >
              {avatarUploading ? t('common.uploading', 'Uploading...') : t('profile.edit.changePhoto', 'Change photo')}
            </button>
            <p className="text-xs text-zinc-500 mt-0.5">{t('profile.edit.photoHint', 'JPG, PNG, WebP. Max 2MB.')}</p>
          </div>
          <input
            ref={avatarInputRef}
            type="file"
