...
const JobCard = memo(function JobCard({ task, onClick, hasApplied }: { task: Task; onClick: () => void; hasApplied?: boolean }) {
  ...
  return (
    <article
      className="bg-white rounded-xl border border-zinc-200 hover:border-zinc-400 hover:shadow-md transition-all cursor-pointer group"
      onClick={onClick}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-xs ${isExpiring ? 'text-amber-700 font-medium' : 'text-zinc-500'}`}>
            {deadlineText}
          </span>
        </div>

        {/* Title */}
        <h3 className="font-semibold text-zinc-900 mb-1.5 line-clamp-2 group-hover:text-zinc-700 transition-colors">
          {task.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-zinc-100">
          ...
          {hasApplied ? (
            <span className="px-4 py-1.5 text-sm font-semibold rounded-lg bg-zinc-100 text-zinc-700 ring-1 ring-zinc-300">
      ...
      <section ref={ref} className="py-8">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            {!loading && tasks.length > 0 && (
              <span className="px-2.5 py-0.5 bg-zinc-900 text-white text-sm font-bold rounded-full">
                {tasks.length}
              </span>
            )}
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-white rounded-xl border border-zinc-200 p-4 animate-pulse">
                ...
                <div className="w-3/4 h-5 bg-zinc-200 rounded mb-2" />
                <div className="w-full h-4 bg-zinc-200 rounded mb-1" />
                ...
                <div className="flex justify-between pt-3 border-t border-zinc-100">
                  ...
                  <div className="w-20 h-8 bg-zinc-200 rounded-lg" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="text-center py-16">
            ...
            <button
              onClick={() => refetch()}
              className="px-4 py-2 bg-zinc-900 text-white text-sm font-semibold rounded-lg hover:bg-zinc-800 transition-colors"
            >
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 mb-1">{t('tasks.noTasks', 'No tasks available')}</h3>
            <p className="text-zinc-500 text-sm">{t('tasks.emptyHint', 'Check back soon for new jobs')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tasks.map((task) => (
