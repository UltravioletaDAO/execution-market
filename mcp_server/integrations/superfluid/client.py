...
class SuperfluidClient:
    ...
    def __init__(
        ...
        self.host = self.w3.eth.contract(
            address=Web3.to_checksum_address(contracts["host"]),
            abi=SUPERFLUID_HOST_ABI
        )
        self.cfa = self.w3.eth.contract(
            address=Web3.to_checksum_address(contracts["cfa"]),
            abi=CFA_ABI
        )
    ...
    async def create_stream(
        ...
        try:
            ...
            if receipt['status'] == 1:
                logger.info(f"Stream created: {receiver}, rate: {flow_rate}, task: {task_id}")
                return tx_hash.hex()
            else:
