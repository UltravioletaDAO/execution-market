"""Explicit hold decision for the Incident Verification internal sample output.

This module advances the Incident Verification adjacent-AAS ladder by exactly one
...
release, emergency response, safety certification, repair/insurance/SLA claims,
official incident reporting, fault/liability assignment, or worker-copyable
incident doctrine.
"""
...
INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA = (
    "city_ops.incident_verification_sample_output_review_decision.v1"
)
...
DECISION_ID = "execution_market.aas.incident_verification.sample_output_review_decision.001"
SCOPE = "internal_admin_incident_verification_sample_output_hold_decision_only"
...
REQUIRED_REVIEW_FINDINGS = [
    ...
    "sample_populates_only_allowed_incident_verification_fields",
    "privacy_redaction_notice_is_present",
    "limitations_and_non_guarantees_are_present",
    "emergency_safety_repair_insurance_sla_official_report_fault_liability_exclusions_are_present",
...
HOLD_REASONS = [
    ...
    "Emergency, safety, repair, insurance, SLA, official-report, fault, and liability claims remain blocked.",
    "Exact-location and raw metadata release remain blocked.",
    "Worker-copyable incident doctrine remains blocked.",
]
...
def build_incident_verification_sample_output_review_decision(
    *, sample_output: dict[str, Any] | None = None
) -> dict[str, Any]:
    """Build a conservative explicit hold decision over the sample output."""

    source = sample_output or load_incident_verification_internal_sample_output()
    ...
    packet = {
        ...
        "source_sample_output_file": INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_FILENAME,
        ...
        "sample_output_boundary": {
            "consumes_only": [INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_FILENAME],
            ...
            "emergency_safety_repair_insurance_sla_official_report_fault_or_liability_claim_allowed": False,
        },
        ...
        "operator_instruction": (
            "This records an explicit hold over the Incident Verification internal sample. "
            ...
            "as emergency, safety, repair, insurance, SLA, official-report, fault, liability, "
...
def write_incident_verification_sample_output_review_decision(
    *, artifact_dir: str | Path | None = None
) -> Path:
    """Persist the Incident Verification sample-output hold decision."""

    packet = build_incident_verification_sample_output_review_decision()
    target_dir = Path(artifact_dir) if artifact_dir is not None else ARTIFACT_DIR
    target_dir.mkdir(parents=True, exist_ok=True)
    path = target_dir / INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME
    path.write_text(json.dumps(packet, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path


def load_incident_verification_sample_output_review_decision(
    *, artifact_dir: str | Path | None = None
) -> dict[str, Any]:
    """Load and validate the Incident Verification sample-output hold decision."""

    source_dir = Path(artifact_dir) if artifact_dir is not None else ARTIFACT_DIR
    path = source_dir / INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME
    with path.open("r", encoding="utf-8") as fh:
        packet = json.load(fh)
    if not isinstance(packet, dict):
        raise CityOpsContractError("Incident Verification sample output review decision must be a JSON object")
...
def _load_source_sample_for_dir(source_dir: Path) -> dict[str, Any]:
    if (source_dir / INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_FILENAME).exists():
        return load_incident_verification_internal_sample_output(artifact_dir=source_dir)
    return load_incident_verification_internal_sample_output()
...
def _assert_source_sample_is_conservative(sample: dict[str, Any]) -> None:
    if sample.get("schema") != INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_SCHEMA:
        raise CityOpsContractError("Incident Verification sample decision source schema drift")
    if sample.get("sample_output_id") != SAMPLE_OUTPUT_ID:
        raise CityOpsContractError("Incident Verification sample decision source id drift")
    if sample.get("scope") != "internal_admin_incident_verification_sample_output_only":
        raise CityOpsContractError("Incident Verification sample decision source scope drift")
    if sample.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("Incident Verification sample decision source offer drift")
    if INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM not in sample.get("safe_to_claim", []):
        raise CityOpsContractError("Incident Verification sample decision source safe claim missing")
    ladder = sample.get("ladder_boundary", {})
    if ladder.get("promotion_allowed") is not False:
        raise CityOpsContractError("Incident Verification sample decision source promoted readiness")
    if ladder.get("next_required_steps_before_promotion") != ["explicit_approval_or_hold_decision"]:
        raise CityOpsContractError("Incident Verification sample decision source next step drift")
    missing_blocked = [
        claim for claim in REQUIRED_SOURCE_BLOCKED_CLAIMS if claim not in sample.get("do_not_claim_yet", [])
    ]
    if missing_blocked:
        raise CityOpsContractError(
            f"Incident Verification sample decision source missing blocked claims: {missing_blocked}"
        )
    for flag in READINESS_FALSE_FLAGS:
        if sample.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(f"Incident Verification sample decision source promoted readiness: {flag}")
    for flag, value in sample.get("sample_output_readiness", {}).items():
        if value is not False:
            raise CityOpsContractError(
                f"Incident Verification sample decision source promoted sample readiness: {flag}"
            )
    ...
    for flag in [
        ...
        if source_contract.get(flag) is not False:
            raise CityOpsContractError(f"Incident Verification sample decision source contract overclaims: {flag}")
    ...
    if sample_output.get("sample_review_status") != "internal_admin_sample_against_schema_gate_not_customer_copy":
        raise CityOpsContractError("Incident Verification sample decision source review status drift")
    if sample_output.get("jurisdiction_specific") is not False:
        raise CityOpsContractError("Incident Verification sample decision source became jurisdiction-specific")
    ...
    for flag in [
        ...
        if reviews.get(flag) is not False:
            raise CityOpsContractError(f"Incident Verification sample decision source promoted review flag: {flag}")
...
def _assert_decision_is_conservative(
    ...
    if packet.get("schema") != INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA:
        raise CityOpsContractError("Incident Verification sample output review decision schema drift")
    if packet.get("decision_id") != DECISION_ID:
        raise CityOpsContractError("Incident Verification sample output review decision id drift")
    if packet.get("scope") != SCOPE:
        raise CityOpsContractError("Incident Verification sample output review decision scope drift")
    if packet.get("decision_status") != DECISION_STATUS:
        raise CityOpsContractError("Incident Verification sample output review decision status drift")
    if packet.get("source_sample_output_id") != source_sample.get("sample_output_id"):
        raise CityOpsContractError("Incident Verification sample output review decision source drift")

    _assert_claim_boundaries(packet.get("safe_to_claim", []), packet.get("do_not_claim_yet", []))
    if INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM not in packet.get("safe_to_claim", []):
        raise CityOpsContractError("Incident Verification sample output review decision safe claim missing")
    missing_blocked = (set(source_sample.get("do_not_claim_yet", [])) | set(DECISION_BLOCKED_CLAIMS)) - set(
        packet.get("do_not_claim_yet", [])
    )
    if missing_blocked:
        raise CityOpsContractError(
            f"Incident Verification sample output review decision missing blocked claims: {sorted(missing_blocked)}"
        )

    ladder = packet.get("ladder_boundary", {})
    if ladder.get("covered_steps") != COVERED_LADDER_STEPS:
        raise CityOpsContractError("Incident Verification sample output review decision covered steps drift")
    if ladder.get("next_required_steps_before_promotion") != NEXT_REQUIRED_LADDER_STEPS:
        raise CityOpsContractError("Incident Verification sample output review decision next steps drift")
    if ladder.get("promotion_allowed") is not False:
        raise CityOpsContractError("Incident Verification sample output review decision promoted ladder")

    if packet.get("review_decision") != REVIEW_DECISION:
        raise CityOpsContractError("Incident Verification sample output review decision changed verdict")
    if packet.get("explicit_hold_decision_recorded") is not True:
        raise CityOpsContractError("Incident Verification sample output review decision missing explicit hold")
    if packet.get("operator_review_recorded") is not True:
        raise CityOpsContractError("Incident Verification sample output review decision missing operator review record")
    for flag in DECISION_READINESS_FALSE_FLAGS:
        if packet.get(flag, packet.get("decision_readiness", {}).get(flag)) is not False:
            raise CityOpsContractError(
                f"Incident Verification sample output review decision promoted readiness: {flag}"
            )
    for flag in READINESS_FALSE_FLAGS:
        if packet.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(
                f"Incident Verification sample output review decision promoted base readiness: {flag}"
            )

    boundary = packet.get("sample_output_boundary", {})
    if boundary.get("consumes_only") != [INCIDENT_VERIFICATION_INTERNAL_SAMPLE_OUTPUT_FILENAME]:
        raise CityOpsContractError("Incident Verification sample output review decision input drift")
    for flag in [
        ...
        if boundary.get(flag) is not False:
            raise CityOpsContractError(f"Incident Verification sample output review decision boundary overclaims: {flag}")
    ...
    if not isinstance(findings, list) or [item.get("finding") for item in findings] != REQUIRED_REVIEW_FINDINGS:
        raise CityOpsContractError("Incident Verification sample output review decision findings drift")
    for item in findings:
        if item.get("verified") is not True:
            raise CityOpsContractError("Incident Verification sample output review decision finding not verified")
        if item.get("approval_granted") is not False or item.get("hold_required") is not True:
            raise CityOpsContractError("Incident Verification sample output review decision finding promoted approval")
    if packet.get("hold_reasons") != HOLD_REASONS:
        raise CityOpsContractError("Incident Verification sample output review decision hold reasons drift")
    if packet.get("still_blocked_claims") != packet.get("do_not_claim_yet"):
        raise CityOpsContractError("Incident Verification sample output review decision blocked claim mirror drift")
...
def _assert_claim_boundaries(safe_to_claim: list[str], do_not_claim_yet: list[str]) -> None:
    ...
    if forbidden:
        raise CityOpsContractError(
            f"Incident Verification sample output review decision has forbidden safe claims: {forbidden}"
        )
    overlap = sorted(safe & blocked)
    if overlap:
        raise CityOpsContractError(
            f"Incident Verification sample output review decision claim overlap: {overlap}"
        )
...
def _assert_no_forbidden_text(packet: dict[str, Any]) -> None:
    ...
    for fragment in FORBIDDEN_TEXT_FRAGMENTS:
        if fragment in serialized:
            raise CityOpsContractError(
                f"Incident Verification sample output review decision forbidden text fragment: {fragment}"
            )
