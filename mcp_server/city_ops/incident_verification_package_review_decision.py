"""Internal Incident Verification package-review decision.

This module implements the recommended no-human/no-live-runtime AAS slice for
Incident Verification. It consumes the existing explicit hold decision and
answers only internal package-review questions: safe internal label, allowed
future evidence fields, forbidden authority language, follow-on/specialist
routing boundaries, and the next gate required before any delivery path exists.
...
INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_SCHEMA = (
    "city_ops.incident_verification_package_review_decision.v1"
)
...
DECISION_ID = "execution_market.aas.incident_verification.package_review_decision.001"
SCOPE = "internal_admin_incident_verification_package_review_decision_only"
SOURCE_POLICY = "consume_only_incident_verification_sample_output_review_decision_json"
...
FORBIDDEN_FIELD_CLASSES = [
    ...
    "private_operator_context_or_precise_private_location",
    "emergency_response_instruction_or_readiness",
    "safety_certification_claim",
    "repair_diagnosis_or_completion_claim",
    "insurance_adjustment_claim",
    "sla_uptime_or_availability_guarantee",
    "official_incident_report_claim",
    "fault_or_liability_assignment",
    "dispatch_instruction_or_worker_assignment",
...
FOLLOW_ON_ESCALATION_BOUNDARIES = [
    {
        "trigger": "immediate danger or safety-critical condition appears in evidence",
        "allowed_decision": "route_to_specialist_or_emergency_channel_outside_execution_market",
        "execution_market_action_authorized": False,
    },
    {
        "trigger": "repair, insurance, official-report, fault, or liability conclusion requested",
        "allowed_decision": "record limitation and require specialist review before any customer-facing claim",
        "execution_market_action_authorized": False,
    },
    {
        "trigger": "evidence is incomplete or stale",
        "allowed_decision": "recommend a bounded follow-on evidence-gathering task only after separate operator review",
        "execution_market_action_authorized": False,
    },
]

NEXT_REQUIRED_GATE = (
    "separate_human_operator_approval_artifact_for_one_exact_incident_verification_text_boundary"
)
...
PACKAGE_REVIEW_BLOCKED_CLAIMS = [
    *SOURCE_DECISION_BLOCKED_CLAIMS,
    "incident_verification_package_label_customer_copy_ready",
    "incident_verification_customer_delivery_ready",
    "incident_verification_publication_ready",
    "incident_verification_public_route_ready",
    "incident_verification_catalog_ready",
    "incident_verification_controlled_pilot_ready",
    "incident_verification_front_door_sku_ready",
    "incident_verification_public_price_ready",
    ...
    "incident_verification_reputation_receipts_ready",
    "incident_verification_live_acontext_ready",
    "incident_verification_runtime_parity_ready",
    ...
    "incident_verification_worker_skill_dna_ready",
    "incident_verification_worker_copyable_doctrine_ready",
]
...
def _load_source_decision(artifact_dir: Path | None = None) -> dict[str, Any]:
    return load_incident_verification_sample_output_review_decision(artifact_dir=artifact_dir)


def _assert_source_decision(decision: dict[str, Any]) -> None:
    if decision.get("schema") != INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA:
        raise CityOpsContractError("incident package review source decision schema drift")
    if decision.get("package_family_id") != PACKAGE_FAMILY_ID:
        raise CityOpsContractError("incident package review source family drift")
    if decision.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("incident package review source offer drift")
    if decision.get("review_decision") != SOURCE_REVIEW_DECISION:
        raise CityOpsContractError("incident package review source decision promoted from hold")
    if decision.get("explicit_hold_decision_recorded") is not True:
        raise CityOpsContractError("incident package review source hold missing")
    if INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM not in decision.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident package review source safe claim missing")
    forbidden_safe = set(decision.get("safe_to_claim", [])) & FORBIDDEN_SAFE_CLAIMS
    if forbidden_safe:
        raise CityOpsContractError(
            f"incident package review source forbidden safe claims: {sorted(forbidden_safe)}"
        )
    ...
    if missing_blocked:
        raise CityOpsContractError(
            f"incident package review source missing blocked claims: {sorted(missing_blocked)}"
        )
    for container in ["readiness", "decision_readiness"]:
        ...
        for flag in REQUIRED_SOURCE_FALSE_FLAGS:
            if flag in flags and flags.get(flag) is not False:
                raise CityOpsContractError(
                    f"incident package review source promoted {container} flag {flag}"
                )
    ...
    for flag in required_false:
        if boundary.get(flag) is not False:
            raise CityOpsContractError(f"incident package review source boundary promoted {flag}")
...
def _build_review_questions() -> list[dict[str, Any]]:
    return [
        ...
        {
            "question": "Which incident evidence fields may remain in a future schema?",
...
def build_incident_verification_package_review_decision(
    *,
    artifact_dir: Path | None = None,
    source_decision: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build the internal/admin Incident Verification package-review decision."""
    ...
    decision = {
        "schema": INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_SCHEMA,
        "decision_id": DECISION_ID,
        "scope": SCOPE,
        "source_policy": SOURCE_POLICY,
        "source_decision_file": INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME,
        ...
        "source_safe_claim": INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM,
        ...
        "package_review_summary": {
            ...
            "follow_on_boundaries_named": True,
            ...
            "queue_dispatch_reputation_runtime_gps_incident_authority_worker_doctrine_approved": False,
        },
        "readiness": {flag: False for flag in PACKAGE_REVIEW_FALSE_FLAGS},
        "still_blocked_claims": do_not_claim_yet,
        "operator_instruction": (
            "Use this as an internal Incident Verification package-review decision only. "
...
def _assert_package_review_decision(decision: dict[str, Any]) -> None:
    if decision.get("schema") != INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_SCHEMA:
        raise CityOpsContractError("incident package review schema drift")
    if decision.get("decision_id") != DECISION_ID:
        raise CityOpsContractError("incident package review id drift")
    if decision.get("scope") != SCOPE:
        raise CityOpsContractError("incident package review scope drift")
    if decision.get("source_policy") != SOURCE_POLICY:
        raise CityOpsContractError("incident package review source policy drift")
    if decision.get("source_decision_file") != INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME:
        raise CityOpsContractError("incident package review source file drift")
    if decision.get("source_decision_schema") != INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA:
        raise CityOpsContractError("incident package review source schema drift")
    if decision.get("source_safe_claim") != INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM:
        raise CityOpsContractError("incident package review source safe claim drift")
    if decision.get("package_family_id") != PACKAGE_FAMILY_ID:
        raise CityOpsContractError("incident package review family drift")
    if decision.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("incident package review offer drift")
    if INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_SAFE_CLAIM not in decision.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident package review safe claim missing")
    if INCIDENT_VERIFICATION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM not in decision.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident package review source safe claim missing")
    ...
    if forbidden_safe:
        raise CityOpsContractError(
            f"incident package review has forbidden safe claims: {sorted(forbidden_safe)}"
        )
    ...
    if missing_blocked:
        raise CityOpsContractError(
            f"incident package review missing blocked claims: {sorted(missing_blocked)}"
        )
    if decision.get("still_blocked_claims") != decision.get("do_not_claim_yet"):
        raise CityOpsContractError("incident package review blocked claims drift")
    if decision.get("package_review_decision") != PACKAGE_REVIEW_DECISION:
        raise CityOpsContractError("incident package review decision promoted")
    if decision.get("selected_internal_label") != SELECTED_INTERNAL_LABEL:
        raise CityOpsContractError("incident package review label drift")
    if decision.get("selected_internal_label_customer_copy_approved") is not False:
        raise CityOpsContractError("incident package label promoted to customer copy")
    if decision.get("allowed_future_customer_output_fields") != ALLOWED_FUTURE_CUSTOMER_OUTPUT_FIELDS:
        raise CityOpsContractError("incident allowed field drift")
    ...
    if decision.get("next_gate_satisfied") is not False:
        raise CityOpsContractError("incident next gate promoted")
    for flag in PACKAGE_REVIEW_FALSE_FLAGS:
        if decision.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(f"incident package review promoted {flag}")
    ...
    required_true = [
        ...
        "follow_on_boundaries_named",
        "next_gate_named",
    ]
    for flag in required_true:
        if summary.get(flag) is not True:
            raise CityOpsContractError(f"incident package review summary drift {flag}")
    required_false = [
    ...
    for flag in required_false:
        if summary.get(flag) is not False:
            raise CityOpsContractError(f"incident package review summary promoted {flag}")
    for row in decision.get("review_questions", []):
        if row.get("approval_granted") is not False:
            raise CityOpsContractError("incident package review question promoted")


def write_incident_verification_package_review_decision(artifact_dir: Path | None = None) -> Path:
    target_dir = artifact_dir or ARTIFACT_DIR
    target_dir.mkdir(parents=True, exist_ok=True)
    decision = build_incident_verification_package_review_decision(artifact_dir=target_dir)
    path = target_dir / INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_FILENAME
    path.write_text(json.dumps(decision, indent=2) + "\n", encoding="utf-8")
    return path


def load_incident_verification_package_review_decision(
    artifact_dir: Path | None = None,
) -> dict[str, Any]:
    source_dir = artifact_dir or ARTIFACT_DIR
    path = source_dir / INCIDENT_VERIFICATION_PACKAGE_REVIEW_DECISION_FILENAME
