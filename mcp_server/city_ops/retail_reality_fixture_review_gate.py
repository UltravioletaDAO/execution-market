"""Retail Reality adjacent-AAS fixture spec and review gate.

This module stages the first next-concept AAS family from the May 21 planning
packet: Retail Reality as a Service. It deliberately stops at an internal
fixture spec and review-gate checklist. It does not create customer copy,
publish a catalog, authorize a pilot, prove live Acontext/runtime parity,
dispatch work, attach ERC-8004 reputation, expose exact GPS/raw metadata, or
create worker-copyable retail doctrine.
"""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any

from .aas_minimum_ladder_template import (
    AAS_MINIMUM_LADDER_TEMPLATE_SAFE_CLAIM,
    READINESS_FALSE_FLAGS,
    REQUIRED_BLOCKED_CLAIMS as TEMPLATE_BLOCKED_CLAIMS,
    REQUIRED_LADDER_STEPS,
)
from .compliance_desk_fixture_review_gate import ARTIFACT_DIR
from .contracts import CityOpsContractError

RETAIL_REALITY_FIXTURE_REVIEW_GATE_SCHEMA = (
    "city_ops.retail_reality_fixture_review_gate.v1"
)
RETAIL_REALITY_FIXTURE_REVIEW_GATE_FILENAME = "retail_reality_fixture_review_gate.json"
RETAIL_REALITY_FIXTURE_REVIEW_GATE_SAFE_CLAIM = (
    "retail_reality_fixture_review_gate_landed"
)

PACKAGE_FAMILY_ID = "retail_reality_as_a_service"
OFFER_ID = "storefront_hours_availability_check"
SCOPE = "internal_admin_retail_reality_fixture_spec_and_review_gate_only"
SOURCE_CONCEPT_DOC = "docs/planning/EXECUTION_MARKET_AAS_NEXT_CONCEPTS_2026_05_21_10PM.md"

COVERED_LADDER_STEPS = [
    "narrow_concierge_offer_card",
    "fixture_spec",
    "review_gate_checklist",
]
NEXT_REQUIRED_LADDER_STEPS = [
    "reviewed_output_schema",
    "local_reviewed_fixture",
    "internal_package_record",
    "coverage_summary_or_read_only_operator_surface",
    "customer_output_schema_gate",
    "internal_sample_output",
    "explicit_approval_or_hold_decision",
]

REQUIRED_EVIDENCE_FIELDS = [
    "storefront_context_photo_or_permitted_visual_snapshot",
    "posted_hours_or_open_closed_state_proof",
    "observation_window",
    "staff_answer_source_type_where_available",
    "availability_or_service_observed_state",
    "discrepancy_summary_between_posted_and_observed_state",
    "uncertainty_note",
    "what_was_not_checked",
]

REQUIRED_OUTPUT_FIELDS = [
    "task_id_or_local_case_reference",
    "offer_type",
    "plain_language_observation_status",
    "observed_open_closed_or_unable_to_determine_state",
    "posted_hours_summary",
    "availability_or_service_state_summary",
    "source_type_summary",
    "discrepancy_summary",
    "observation_window_summary",
    "what_was_checked",
    "what_was_not_checked",
    "limitations_and_non_guarantees",
    "recommended_next_action",
    "operator_review_notice",
]

REVIEW_GATE_CHECKS = [
    "source_concept_boundary_matches_retail_reality_storefront_hours_availability_check",
    "evidence_contract_requires_one_storefront_one_window_one_question",
    "posted_hours_and_observed_state_are_required_but_not_treated_as_permanent_status",
    "staff_answer_preserved_as_source_type_not_authority",
    "availability_observation_preserved_without_inventory_guarantee",
    "discrepancy_summary_and_uncertainty_note_preserved",
    "privacy_redaction_required_before_any_customer_language",
    "exact_gps_and_raw_metadata_blocked",
    "brand_compliance_employee_performance_and_consumer_safety_claims_blocked",
    "operator_review_required_before_fixture_acceptance",
    "publication_customer_delivery_dispatch_reputation_and_worker_doctrine_blocked",
]

RETAIL_REALITY_SPECIFIC_BLOCKED_CLAIMS = [
    "permanent_business_status_claim",
    "inventory_guarantee",
    "brand_compliance_certification",
    "employee_performance_judgment",
    "consumer_safety_claim",
    "worker_copyable_retail_doctrine",
]

REQUIRED_BLOCKED_CLAIMS = [
    *TEMPLATE_BLOCKED_CLAIMS,
    *RETAIL_REALITY_SPECIFIC_BLOCKED_CLAIMS,
]

FORBIDDEN_SAFE_CLAIMS = set(REQUIRED_BLOCKED_CLAIMS) | {
    "retail_reality_customer_ready",
    "retail_reality_catalog_ready",
    "retail_reality_dispatch_ready",
    "retail_reality_reputation_ready",
    "retail_reality_worker_doctrine_ready",
    "storefront_status_guaranteed",
    "inventory_available_guaranteed",
}



def build_retail_reality_fixture_review_gate() -> dict[str, Any]:
    """Build the internal fixture spec and review-gate checklist.

    The returned artifact proves only that the Retail Reality fixture boundary
    exists. It does not prove reviewed fixtures, customer output, approval,
    publication, dispatch, reputation, live runtime, or worker doctrine.
    """

    gate = {
        "schema": RETAIL_REALITY_FIXTURE_REVIEW_GATE_SCHEMA,
        "gate_id": "execution_market.aas.retail_reality.fixture_review_gate.2026_05_23",
        "scope": SCOPE,
        "package_family_id": PACKAGE_FAMILY_ID,
        "source_concept_doc": SOURCE_CONCEPT_DOC,
        "source_concept_section": "Retail Reality AAS — Storefront hours + availability check",
        "source_template_safe_claims_inherited": [
            AAS_MINIMUM_LADDER_TEMPLATE_SAFE_CLAIM
        ],
        "safe_to_claim": [
            RETAIL_REALITY_FIXTURE_REVIEW_GATE_SAFE_CLAIM,
            AAS_MINIMUM_LADDER_TEMPLATE_SAFE_CLAIM,
        ],
        "do_not_claim_yet": _dedupe(REQUIRED_BLOCKED_CLAIMS),
        "ladder_boundary": {
            "required_full_ladder": list(REQUIRED_LADDER_STEPS),
            "covered_steps": list(COVERED_LADDER_STEPS),
            "next_required_steps_before_promotion": list(NEXT_REQUIRED_LADDER_STEPS),
            "promotion_allowed": False,
        },
        "fixture_spec": {
            "offer_id": OFFER_ID,
            "offer_label": "Storefront Hours + Availability Check",
            "family_label": "Retail Reality as a Service",
            "caas_source_pattern": "verify_place + counter_question + site_audit",
            "source_caas_offer": "one_storefront_hours_availability_observation",
            "fixture_status": "spec_only_no_reviewed_fixture_yet",
            "operator_review_required": True,
            "customer_copy_changed": False,
            "phase_1_sellable_claim_allowed": False,
            "automation_claim_allowed": False,
            "required_evidence_fields": list(REQUIRED_EVIDENCE_FIELDS),
            "reviewed_output_schema_draft": {
                "status": "draft_internal_only_not_customer_output",
                "required_fields": list(REQUIRED_OUTPUT_FIELDS),
                "forbidden_fields": [
                    "exact_gps_coordinates",
                    "raw_metadata_blob",
                    "raw_transcript_as_authority",
                    "private_operator_context",
                    "private_staff_identity",
                    "employee_performance_judgment",
                    "permanent_business_status_claim",
                    "inventory_guarantee_language",
                    "brand_compliance_certification_claim",
                    "consumer_safety_claim",
                    "dispatch_instruction_or_assignment",
                    "erc8004_reputation_receipt",
                    "worker_copyable_retail_doctrine",
                ],
            },
            "fixture_acceptance_gate": {
                "requires_local_reviewed_fixture": True,
                "requires_privacy_redaction_review": True,
                "requires_non_guarantee_language_review": True,
                "requires_operator_review_record": True,
                "preserves_safe_and_blocked_claims": True,
                "allows_customer_delivery": False,
                "allows_publication": False,
            },
        },
        "review_gate_checklist": [
            {
                "check_id": check,
                "required": True,
                "status": "pending_future_review",
                "blocks_promotion_until_passed": True,
            }
            for check in REVIEW_GATE_CHECKS
        ],
        "readiness": {flag: False for flag in READINESS_FALSE_FLAGS},
        "operator_instruction": (
            "Use this only as the Retail Reality fixture boundary. The next valid "
            "step is a local reviewed fixture plus reviewed-output schema, not "
            "customer copy, catalog routing, dispatch, reputation, live memory, "
            "or worker-copyable retail doctrine."
        ),
        "next_smallest_proof": (
            "Create one local reviewed Retail Reality fixture for a scoped storefront "
            "hours + availability check that fills this evidence contract while keeping "
            "all promotion and customer/public readiness flags false."
        ),
    }
    _assert_gate_is_conservative(gate)
    return gate



def write_retail_reality_fixture_review_gate(
    *, artifact_dir: str | Path | None = None
) -> Path:
    """Persist the Retail Reality fixture review gate."""

    gate = build_retail_reality_fixture_review_gate()
    target_dir = Path(artifact_dir) if artifact_dir is not None else ARTIFACT_DIR
    target_dir.mkdir(parents=True, exist_ok=True)
    path = target_dir / RETAIL_REALITY_FIXTURE_REVIEW_GATE_FILENAME
    path.write_text(json.dumps(gate, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path



def load_retail_reality_fixture_review_gate(
    *, artifact_dir: str | Path | None = None
) -> dict[str, Any]:
    """Load and validate the persisted Retail Reality fixture review gate."""

    source_dir = Path(artifact_dir) if artifact_dir is not None else ARTIFACT_DIR
    path = source_dir / RETAIL_REALITY_FIXTURE_REVIEW_GATE_FILENAME
    with path.open("r", encoding="utf-8") as fh:
        gate = json.load(fh)
    if not isinstance(gate, dict):
        raise CityOpsContractError("Retail Reality fixture review gate must be a JSON object")
    _assert_gate_is_conservative(gate)
    return gate



def _assert_gate_is_conservative(gate: dict[str, Any]) -> None:
    if gate.get("schema") != RETAIL_REALITY_FIXTURE_REVIEW_GATE_SCHEMA:
        raise CityOpsContractError("Retail Reality fixture review gate schema drift")
    if gate.get("scope") != SCOPE:
        raise CityOpsContractError("Retail Reality fixture review gate scope drift")
    if gate.get("package_family_id") != PACKAGE_FAMILY_ID:
        raise CityOpsContractError("Retail Reality fixture review gate family drift")
    if gate.get("source_concept_doc") != SOURCE_CONCEPT_DOC:
        raise CityOpsContractError("Retail Reality fixture review gate source doc drift")
    if set(gate.get("safe_to_claim", [])) & FORBIDDEN_SAFE_CLAIMS:
        raise CityOpsContractError("Retail Reality fixture review gate has forbidden safe claims")

    missing_blocked = set(REQUIRED_BLOCKED_CLAIMS) - set(gate.get("do_not_claim_yet", []))
    if missing_blocked:
        raise CityOpsContractError(
            f"Retail Reality fixture review gate missing blocked claims: {sorted(missing_blocked)}"
        )

    ladder = gate.get("ladder_boundary", {})
    if ladder.get("covered_steps") != COVERED_LADDER_STEPS:
        raise CityOpsContractError("Retail Reality fixture review gate covered steps drift")
    if ladder.get("next_required_steps_before_promotion") != NEXT_REQUIRED_LADDER_STEPS:
        raise CityOpsContractError("Retail Reality fixture review gate next steps drift")
    if ladder.get("promotion_allowed") is not False:
        raise CityOpsContractError("Retail Reality fixture review gate promoted readiness")

    readiness = gate.get("readiness", {})
    for flag in READINESS_FALSE_FLAGS:
        if readiness.get(flag) is not False:
            raise CityOpsContractError(
                f"Retail Reality fixture review gate promoted readiness: {flag}"
            )

    spec = gate.get("fixture_spec")
    if not isinstance(spec, dict):
        raise CityOpsContractError("Retail Reality fixture spec must be an object")
    if spec.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("Retail Reality fixture spec offer drift")
    if spec.get("operator_review_required") is not True:
        raise CityOpsContractError("Retail Reality fixture spec must require operator review")
    for flag in (
        "customer_copy_changed",
        "phase_1_sellable_claim_allowed",
        "automation_claim_allowed",
    ):
        if spec.get(flag) is not False:
            raise CityOpsContractError(f"Retail Reality fixture spec promoted {flag}")
    if not set(REQUIRED_EVIDENCE_FIELDS).issubset(
        set(spec.get("required_evidence_fields", []))
    ):
        raise CityOpsContractError("Retail Reality fixture spec lost required evidence")

    schema = spec.get("reviewed_output_schema_draft", {})
    if not set(REQUIRED_OUTPUT_FIELDS).issubset(set(schema.get("required_fields", []))):
        raise CityOpsContractError("Retail Reality fixture spec lost output fields")
    forbidden_fields = set(schema.get("forbidden_fields", []))
    for forbidden in [
        "exact_gps_coordinates",
        "raw_metadata_blob",
        "private_staff_identity",
        "employee_performance_judgment",
        "permanent_business_status_claim",
        "inventory_guarantee_language",
        "brand_compliance_certification_claim",
        "consumer_safety_claim",
        "worker_copyable_retail_doctrine",
    ]:
        if forbidden not in forbidden_fields:
            raise CityOpsContractError("Retail Reality fixture spec lost forbidden fields")

    acceptance = spec.get("fixture_acceptance_gate", {})
    for flag in [
        "requires_local_reviewed_fixture",
        "requires_privacy_redaction_review",
        "requires_non_guarantee_language_review",
        "requires_operator_review_record",
        "preserves_safe_and_blocked_claims",
    ]:
        if acceptance.get(flag) is not True:
            raise CityOpsContractError(f"Retail Reality fixture acceptance lost {flag}")
    for flag in ["allows_customer_delivery", "allows_publication"]:
        if acceptance.get(flag) is not False:
            raise CityOpsContractError(
                f"Retail Reality fixture acceptance promoted readiness: {flag}"
            )

    checklist = gate.get("review_gate_checklist")
    if not isinstance(checklist, list) or [
        item.get("check_id") for item in checklist
    ] != REVIEW_GATE_CHECKS:
        raise CityOpsContractError("Retail Reality fixture review checklist drift")
    for item in checklist:
        if item.get("required") is not True or item.get("blocks_promotion_until_passed") is not True:
            raise CityOpsContractError("Retail Reality fixture checklist no longer blocks promotion")
        if item.get("status") != "pending_future_review":
            raise CityOpsContractError("Retail Reality fixture checklist status drift")



def _dedupe(values: list[str]) -> list[str]:
    seen: set[str] = set()
    out: list[str] = []
    for value in values:
        if value not in seen:
            seen.add(value)
            out.append(value)
    return out
