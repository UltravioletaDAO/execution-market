"""Incident Verification approval-request read surface.

This module consumes the pending Incident Verification human-operator approval
...
delivery, not publication, not a public route/catalog/pilot, not pricing, not
queue launch, not dispatch, not reputation, not live Acontext/runtime parity,
not exact GPS/raw metadata exposure, not raw transcript authority, not incident
response/safety/repair/insurance/SLA/official-report/fault-liability authority,
and not worker-copyable doctrine.
"""
...
INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_SCHEMA = (
    "city_ops.incident_verification_approval_request_read_surface.v1"
)
INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_FILENAME = (
    "incident_verification_approval_request_read_surface.json"
)
INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_SAFE_CLAIM = (
    "incident_verification_approval_request_read_surface_landed"
)

SURFACE_ID = "execution_market.aas.incident_verification.approval_request_read_surface.001"
SCOPE = "internal_admin_incident_verification_approval_request_read_surface_only"
...
SURFACE_FALSE_FLAGS = {
    ...
    "surface_authorizes_raw_transcript_authority": False,
    "surface_authorizes_emergency_safety_repair_insurance_sla_official_report_or_fault_liability_claims": False,
...
SURFACE_BLOCKED_CLAIMS = [
    *SOURCE_REQUEST_REQUIRED_BLOCKED_CLAIMS,
    "incident_verification_approval_request_read_surface_is_human_approval",
    "incident_verification_approval_request_read_surface_satisfies_pre_approval_checks",
    "incident_verification_approval_request_read_surface_satisfies_redactions_or_authority_review",
    "incident_verification_approval_request_read_surface_authorizes_delivery_path",
    "incident_verification_approval_request_read_surface_authorizes_customer_delivery",
    "incident_verification_approval_request_read_surface_authorizes_publication",
    "incident_verification_approval_request_read_surface_authorizes_public_route_or_catalog",
    "incident_verification_approval_request_read_surface_authorizes_controlled_pilot_or_sku",
    "incident_verification_approval_request_read_surface_authorizes_public_price_or_quote",
    "incident_verification_approval_request_read_surface_authorizes_queue_or_dispatch",
    "incident_verification_approval_request_read_surface_authorizes_reputation_or_runtime",
    "incident_verification_approval_request_read_surface_authorizes_exact_gps_or_raw_metadata",
    "incident_verification_approval_request_read_surface_authorizes_raw_transcript_authority",
    ...
    "incident_verification_approval_request_read_surface_authorizes_worker_skill_dna_or_copyable_doctrine",
]
...
REVIEW_QUEUE_ITEMS = [
    "read_exact_incident_boundary",
    "verify_source_request_digest_before_review",
    "verify_pre_approval_checks_with_external_evidence_later",
    "verify_redactions_authority_limits_and_private-location_handling_later",
    "verify_raw_transcript_is_not_used_as_authority_later",
    "record_separate_human_operator_approval_artifact_if_and_only_if_approved",
    "keep_customer_delivery_publication_queue_dispatch_reputation_runtime_incident_authority_and_worker_doctrine_blocked",
]

DELIVERY_PATH_FALSE_FLAGS = [
    "path_recorded",
...
def _load_source_request(artifact_dir: Path | None = None) -> dict[str, Any]:
    return load_incident_verification_human_operator_approval_request(artifact_dir=artifact_dir)


def _assert_source_request_is_conservative(request: dict[str, Any]) -> None:
    if request.get("schema") != INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SCHEMA:
        raise CityOpsContractError("incident approval request read surface source schema drift")
    if request.get("request_id") != REQUEST_ID:
        raise CityOpsContractError("incident approval request read surface source id drift")
    if request.get("package_family_id") != PACKAGE_FAMILY_ID:
        raise CityOpsContractError("incident approval request read surface source family drift")
    if request.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("incident approval request read surface source offer drift")
    if request.get("approval_request_status") != APPROVAL_REQUEST_STATUS:
        raise CityOpsContractError("incident approval request read surface source status drift")
    if INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SAFE_CLAIM not in request.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident approval request read surface source safe claim missing")
    forbidden_safe = set(request.get("safe_to_claim", [])) & FORBIDDEN_SAFE_CLAIMS
    if forbidden_safe:
        raise CityOpsContractError(
            f"incident approval request read surface source forbidden safe claims: {sorted(forbidden_safe)}"
        )
    ...
    if missing_blocked:
        raise CityOpsContractError(
            f"incident approval request read surface source missing blocked claims: {sorted(missing_blocked)}"
        )
    if request.get("selected_text_boundary_count") != 1:
        raise CityOpsContractError("incident approval request read surface source boundary count drift")
    boundary = request.get("selected_text_boundary", {})
    if boundary.get("key") != SELECTED_TEXT_BOUNDARY_KEY:
        raise CityOpsContractError("incident approval request read surface source boundary key drift")
    if boundary.get("candidate_text_boundary") != "internal_package_label_only":
        raise CityOpsContractError("incident approval request read surface source boundary type drift")
    for flag in [
        "selected_text_boundary_approved",
        "human_operator_approval_recorded",
        ...
        if boundary.get(flag) is not False:
            raise CityOpsContractError(
                f"incident approval request read surface source boundary promoted {flag}"
            )
    for flag, expected in REQUEST_FALSE_TOP_LEVEL_FLAGS.items():
        if request.get(flag) is not expected:
            raise CityOpsContractError(f"incident approval request read surface source promoted {flag}")
    for flag in REQUEST_READINESS_FALSE_FLAGS:
        if request.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(
                f"incident approval request read surface source promoted readiness {flag}"
            )
        if boundary.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(
                f"incident approval request read surface source boundary promoted readiness {flag}"
            )
    if [item.get("check") for item in request.get("pre_approval_checks", [])] != REQUIRED_PRE_APPROVAL_CHECKS:
        raise CityOpsContractError("incident approval request read surface source pre-check drift")
    for item in request.get("pre_approval_checks", []):
        if item.get("required_before_human_approval") is not True:
            raise CityOpsContractError("incident approval request read surface source pre-check not required")
        for flag in [
            "passed_here",
            "approval_granted",
            "customer_delivery_allowed",
            ...
            if item.get(flag) is not False:
                raise CityOpsContractError(
                    f"incident approval request read surface source pre-check promoted {flag}"
                )
    if [
        item.get("check") for item in request.get("redaction_and_authority_requirements", [])
    ] != REDACTION_AND_AUTHORITY_REQUIREMENTS:
        raise CityOpsContractError("incident approval request read surface source redaction drift")
    for item in request.get("redaction_and_authority_requirements", []):
        if item.get("required_before_human_approval") is not True:
            raise CityOpsContractError("incident approval request read surface source redaction not required")
        for flag in [
            "passed_here",
            ...
            if item.get(flag) is not False:
                raise CityOpsContractError(
                    f"incident approval request read surface source redaction promoted {flag}"
                )
    path = request.get("authorized_delivery_path", {})
    if path.get("path") != AUTHORIZED_DELIVERY_PATH:
        raise CityOpsContractError("incident approval request read surface source delivery path drift")
    for flag in DELIVERY_PATH_FALSE_FLAGS:
        if path.get(flag) is not False:
            raise CityOpsContractError(
                f"incident approval request read surface source delivery path promoted {flag}"
            )
...
def _build_operator_cards(request: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "status": "none_recorded_no_delivery_no_publication_no_dispatch_no_incident_authority",
...
def build_incident_verification_approval_request_read_surface(
    *,
    ...
    do_not_claim_yet = _dedupe([*SURFACE_BLOCKED_CLAIMS, *request.get("do_not_claim_yet", [])])

    surface: dict[str, Any] = {
        "schema": INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_SCHEMA,
        "surface_id": SURFACE_ID,
        "scope": SCOPE,
        "surface_status": SURFACE_STATUS,
        "package_family_id": PACKAGE_FAMILY_ID,
        "offer_id": OFFER_ID,
        "source_request_file": INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_FILENAME,
        "source_request_id": request["request_id"],
        "source_request_schema": request["schema"],
        "source_request_digest_sha256": _canonical_digest(request),
        "source_safe_claim": INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SAFE_CLAIM,
        ...
        "mount_contract": {
            ...
            "method": "GET",
            "suggested_internal_path": "/internal/admin/aas/incident-verification/approval-request-review",
            "network_route_registered": False,
            "response_fields": [
                "approval_request_snapshot",
                "operator_cards",
        ...
        **SURFACE_FALSE_FLAGS,
        ...
        "operator_instruction": (
            ...
            "Incident Verification approval request. It is not the approval record. "
            ...
            "reputation, live runtime parity, exact GPS/raw metadata release, raw "
            "transcript authority, incident response/safety/repair/insurance/SLA/"
            "official-report/fault-liability claims, or worker doctrine."
        ),
        "next_smallest_proof": (
            "A real human operator may later create a separate approval record for "
            "this exact selected Incident Verification boundary only, with external "
            "review evidence, passed redactions, an explicit delivery-path decision, "
            "incident-authority limits, and still-blocked claims."
        ),
    }
...
def _assert_read_surface(surface: dict[str, Any], *, source_request: dict[str, Any]) -> None:
    ...
    if surface.get("schema") != INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_SCHEMA:
        raise CityOpsContractError("incident approval request read surface schema drift")
    if surface.get("surface_id") != SURFACE_ID:
        raise CityOpsContractError("incident approval request read surface id drift")
    if surface.get("scope") != SCOPE:
        raise CityOpsContractError("incident approval request read surface scope drift")
    if surface.get("surface_status") != SURFACE_STATUS:
        raise CityOpsContractError("incident approval request read surface status drift")
    if surface.get("source_request_file") != INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_FILENAME:
        raise CityOpsContractError("incident approval request read surface source file drift")
    if surface.get("source_request_id") != source_request.get("request_id"):
        raise CityOpsContractError("incident approval request read surface source id mismatch")
    if surface.get("source_request_schema") != INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SCHEMA:
        raise CityOpsContractError("incident approval request read surface source schema drift")
    if surface.get("source_safe_claim") != INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SAFE_CLAIM:
        raise CityOpsContractError("incident approval request read surface source safe claim drift")
    if surface.get("source_request_digest_sha256") != _canonical_digest(source_request):
        raise CityOpsContractError("incident approval request read surface source digest drift")
    if INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_SAFE_CLAIM not in surface.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident approval request read surface safe claim missing")
    if INCIDENT_VERIFICATION_HUMAN_OPERATOR_APPROVAL_REQUEST_SAFE_CLAIM not in surface.get(
        "safe_to_claim", []
    ):
        raise CityOpsContractError("incident approval request read surface source safe claim missing")
    ...
    if forbidden_safe:
        raise CityOpsContractError(
            f"incident approval request read surface has forbidden safe claims: {sorted(forbidden_safe)}"
        )
    overlap = set(surface.get("safe_to_claim", [])) & set(surface.get("do_not_claim_yet", []))
    if overlap:
        raise CityOpsContractError(f"incident approval request read surface claim overlap: {sorted(overlap)}")
    missing_blocked = set(SURFACE_BLOCKED_CLAIMS) - set(surface.get("do_not_claim_yet", []))
    if missing_blocked:
        raise CityOpsContractError(
            f"incident approval request read surface missing blocked claims: {sorted(missing_blocked)}"
        )
    if surface.get("still_blocked_claims") != surface.get("do_not_claim_yet"):
        raise CityOpsContractError("incident approval request read surface blocked claims drift")

    snapshot = surface.get("approval_request_snapshot", {})
    if snapshot.get("approval_request_status") != APPROVAL_REQUEST_STATUS:
        raise CityOpsContractError("incident approval request read surface snapshot status drift")
    if snapshot.get("selected_text_boundary_count") != 1:
        raise CityOpsContractError("incident approval request read surface snapshot boundary count drift")
    if snapshot.get("selected_text_boundary_key") != SELECTED_TEXT_BOUNDARY_KEY:
        raise CityOpsContractError("incident approval request read surface snapshot boundary drift")
    for flag in [
        "human_operator_approval_recorded",
        "selected_text_boundary_approved",
        ...
        if snapshot.get(flag) is not False:
            raise CityOpsContractError(f"incident approval request read surface snapshot promoted {flag}")

    for flag, expected in SURFACE_FALSE_FLAGS.items():
        if surface.get(flag) is not expected:
            raise CityOpsContractError(f"incident approval request read surface promoted {flag}")
        if surface.get("surface_flags", {}).get(flag) is not expected:
            raise CityOpsContractError(f"incident approval request read surface flag summary promoted {flag}")
    for flag in REQUEST_READINESS_FALSE_FLAGS:
        if surface.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(f"incident approval request read surface promoted readiness {flag}")
    ...
    if access.get("audience") != "internal_admin_only" or access.get("requires_admin_context") is not True:
        raise CityOpsContractError("incident approval request read surface access drift")
    for flag in ["network_route_registered", "public_route_registered", "customer_visible", "worker_visible", "dispatch_enabled"]:
        if access.get(flag) is not False:
            raise CityOpsContractError(f"incident approval request read surface access promoted {flag}")
    mount = surface.get("mount_contract", {})
    if mount.get("mount_status") != MOUNT_STATUS:
        raise CityOpsContractError("incident approval request read surface mount status drift")
    if mount.get("network_route_registered") is not False:
        raise CityOpsContractError("incident approval request read surface registered network route")

    cards = surface.get("operator_cards")
    if not isinstance(cards, list) or len(cards) != 6:
        raise CityOpsContractError("incident approval request read surface cards drift")
    ...
    for required_card in [
        ...
        if required_card not in cards_by_name:
            raise CityOpsContractError("incident approval request read surface missing card")
    pending = cards_by_name["pending_boundary"].get("values", {})
    for flag in ["selected_text_boundary_approved", "human_operator_approval_recorded", "incident_authority_claim_authorized"]:
        if pending.get(flag) is not False:
            raise CityOpsContractError(f"incident approval request read surface pending card promoted {flag}")
    for item in cards_by_name["pre_approval_checks"].get("values", []):
        for flag in [
            ...
            if item.get(flag) is not False:
                raise CityOpsContractError(f"incident approval request read surface pre-check card promoted {flag}")
    for item in cards_by_name["redaction_and_authority_requirements"].get("values", []):
        for flag in ["passed_here", "authorizes_delivery_or_publication", "authorizes_incident_authority_claim"]:
            if item.get(flag) is not False:
                raise CityOpsContractError(f"incident approval request read surface redaction card promoted {flag}")
    delivery_values = cards_by_name["authorized_delivery_path"].get("values", {})
    for flag in DELIVERY_PATH_FALSE_FLAGS:
        if delivery_values.get(flag) is not False:
            raise CityOpsContractError(f"incident approval request read surface delivery card promoted {flag}")
    for item in cards_by_name["review_queue"].get("values", []):
        if item.get("requires_future_human_operator_action") is not True:
            raise CityOpsContractError("incident approval request read surface review queue softened")
        for flag in ["satisfied_by_this_surface", "approval_granted_by_this_surface"]:
            if item.get(flag) is not False:
                raise CityOpsContractError(f"incident approval request read surface review queue promoted {flag}")
...
def _assert_no_private_coordinate_or_authority_overclaim(surface: dict[str, Any]) -> None:
    ...
    for substring in forbidden_substrings:
        if substring in serialized:
            raise CityOpsContractError(
                "incident approval request read surface leaked coordinate/private authority overclaim"
            )


def write_incident_verification_approval_request_read_surface(artifact_dir: Path | None = None) -> Path:
    target_dir = artifact_dir or ARTIFACT_DIR
    target_dir.mkdir(parents=True, exist_ok=True)
    surface = build_incident_verification_approval_request_read_surface(artifact_dir=target_dir)
    path = target_dir / INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_FILENAME
    path.write_text(json.dumps(surface, indent=2) + "\n", encoding="utf-8")
    return path


def load_incident_verification_approval_request_read_surface(
    artifact_dir: Path | None = None,
) -> dict[str, Any]:
    source_dir = artifact_dir or ARTIFACT_DIR
    path = source_dir / INCIDENT_VERIFICATION_APPROVAL_REQUEST_READ_SURFACE_FILENAME
    with path.open("r", encoding="utf-8") as fh:
        surface = json.load(fh)
    if not isinstance(surface, dict):
        raise CityOpsContractError("incident approval request read surface must be JSON object")
