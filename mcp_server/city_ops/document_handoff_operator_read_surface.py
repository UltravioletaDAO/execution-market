"""Document / Handoff adjacent-AAS internal/admin read surface.
...
publish worker-copyable document handoff doctrine.
"""
...
DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_SCHEMA = (
    "city_ops.document_handoff_operator_read_surface.v1"
)
DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_FILENAME = (
    "document_handoff_operator_read_surface.json"
)
DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_SAFE_CLAIM = (
    "document_handoff_operator_read_surface_landed"
)

SURFACE_ID = "execution_market.aas.document_handoff.operator_read_surface.001"
SCOPE = "internal_admin_document_handoff_read_surface_only"
...
READ_SURFACE_BLOCKED_CLAIMS = [
    ...
    "document_handoff_internal_sample_output_ready",
    "document_handoff_operator_approval_ready",
]
...
FORBIDDEN_SAFE_CLAIMS = set(READ_SURFACE_BLOCKED_CLAIMS) | {
    ...
    "legal_service",
    "legal_sufficiency",
    "filing_success_or_acceptance_claim",
    "official_handoff_acceptance",
    "custody_guarantee",
    "live_acontext_ready",
...
def build_document_handoff_operator_read_surface(
    *, package_record: dict[str, Any] | None = None
) -> dict[str, Any]:
    """Build the read-only internal/admin operator surface.

    The surface consumes only the Document / Handoff internal package record and
    reshapes it into operator cards. It proves read-only visibility, not product
    readiness or public/customer exposure.
    """

    source_record = package_record or load_document_handoff_internal_package_record()
    ...
    surface = {
        "schema": DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_SCHEMA,
        ...
        "source_package_file": DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_FILENAME,
        ...
        "derived_from": {
            "read_only": True,
            "source_artifacts": [DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_FILENAME],
            "consumes_only": [DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_FILENAME],
            "forbidden_inputs": [
                "raw_transcript",
                "raw_review_fixture",
                "unreviewed_memory",
        ...
        "mount_contract": {
            ...
            "method": "GET",
            "suggested_internal_path": "/internal/admin/aas/document-handoff/operator-read-surface",
        ...
        "coverage_summary": {
            ...
            "source_bounded_receipt_blocks_promotion": True,
            "custody_scope_blocks_promotion": True,
        ...
        "operator_instruction": (
            ...
            "Document / Handoff package record. Do not treat it as customer copy, a "
        ...
        "next_smallest_proof": (
            "Create a Document / Handoff customer-output schema gate that consumes this "
            "surface and still keeps publication, delivery, dispatch, reputation, "
            "runtime, privacy, legal/notarial/acceptance/custody, and worker-doctrine readiness false."
        ),
    }
...
def write_document_handoff_operator_read_surface(
    *, artifact_dir: str | Path | None = None
) -> Path:
    """Persist the Document / Handoff internal/admin read surface."""
    ...
    path = target_dir / DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_FILENAME
    path.write_text(json.dumps(surface, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return path


def load_document_handoff_operator_read_surface(
    *, artifact_dir: str | Path | None = None
) -> dict[str, Any]:
    """Load and validate the persisted Document / Handoff read surface."""

    source_dir = Path(artifact_dir) if artifact_dir is not None else ARTIFACT_DIR
    path = source_dir / DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_FILENAME
    with path.open("r", encoding="utf-8") as fh:
        surface = json.load(fh)
    if not isinstance(surface, dict):
        raise CityOpsContractError("Document / Handoff operator read surface must be a JSON object")
...
def _load_source_package_record_for_dir(source_dir: Path) -> dict[str, Any]:
    if (source_dir / DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_FILENAME).exists():
        return load_document_handoff_internal_package_record(artifact_dir=source_dir)
    return load_document_handoff_internal_package_record()
...
def _assert_source_package_record_is_conservative(record: dict[str, Any]) -> None:
    if record.get("schema") != DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_SCHEMA:
        raise CityOpsContractError("Document / Handoff read surface source package schema drift")
    if record.get("package_id") != PACKAGE_ID:
        raise CityOpsContractError("Document / Handoff read surface source package id drift")
    if record.get("package_family_id") != PACKAGE_FAMILY_ID:
        raise CityOpsContractError("Document / Handoff read surface source family drift")
    if record.get("offer_id") != OFFER_ID:
        raise CityOpsContractError("Document / Handoff read surface source offer drift")
    if DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_SAFE_CLAIM not in record.get("safe_to_claim", []):
        raise CityOpsContractError("Document / Handoff read surface source safe claim missing")
    if record.get("ladder_boundary", {}).get("promotion_allowed") is not False:
        raise CityOpsContractError("Document / Handoff read surface source promoted readiness")
    for flag in READINESS_FALSE_FLAGS:
        if record.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(f"Document / Handoff read surface source promoted readiness: {flag}")
    package = record.get("internal_package_record")
    if not isinstance(package, dict):
        raise CityOpsContractError("Document / Handoff read surface source package payload missing")
    for false_flag in (
        ...
        if package.get(false_flag) is not False:
            raise CityOpsContractError(f"Document / Handoff read surface source promoted {false_flag}")
...
def _assert_read_surface_is_conservative(
    ...
    if surface.get("schema") != DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_SCHEMA:
        raise CityOpsContractError("Document / Handoff operator read surface schema drift")
    if surface.get("surface_id") != SURFACE_ID:
        raise CityOpsContractError("Document / Handoff operator read surface id drift")
    if surface.get("scope") != SCOPE:
        raise CityOpsContractError("Document / Handoff operator read surface scope drift")
    if surface.get("surface_status") != SURFACE_STATUS:
        raise CityOpsContractError("Document / Handoff operator read surface status drift")
    if surface.get("source_package_id") != source_record.get("package_id"):
        raise CityOpsContractError("Document / Handoff operator read surface source package drift")
    ...
    if DOCUMENT_HANDOFF_OPERATOR_READ_SURFACE_SAFE_CLAIM not in surface.get("safe_to_claim", []):
        raise CityOpsContractError("Document / Handoff operator read surface safe claim missing")
    if DOCUMENT_HANDOFF_INTERNAL_PACKAGE_RECORD_SAFE_CLAIM not in surface.get("safe_to_claim", []):
        raise CityOpsContractError("Document / Handoff operator read surface source claim missing")
    ...
    if missing_blocked:
        raise CityOpsContractError(
            f"Document / Handoff operator read surface missing blocked claims: {sorted(missing_blocked)}"
        )

    ladder = surface.get("ladder_boundary", {})
    if ladder.get("covered_steps") != COVERED_LADDER_STEPS:
        raise CityOpsContractError("Document / Handoff operator read surface covered steps drift")
    if ladder.get("next_required_steps_before_promotion") != NEXT_REQUIRED_LADDER_STEPS:
        raise CityOpsContractError("Document / Handoff operator read surface next steps drift")
    if ladder.get("promotion_allowed") is not False:
        raise CityOpsContractError("Document / Handoff operator read surface promoted readiness")
    ...
    for section_name, flags in (
        ...
        for flag in flags:
            if section.get(flag) is not False:
                raise CityOpsContractError(f"Document / Handoff operator read surface {section_name} overclaims {flag}")
    ...
    if access.get("audience") != "internal_admin_only" or access.get("requires_admin_context") is not True:
        raise CityOpsContractError("Document / Handoff operator read surface access policy drift")
    if surface.get("mount_contract", {}).get("network_route_registered") is not False:
        raise CityOpsContractError("Document / Handoff operator read surface registered network route")

    for flag in READINESS_FALSE_FLAGS:
        if surface.get("readiness", {}).get(flag) is not False:
            raise CityOpsContractError(f"Document / Handoff operator read surface promoted readiness: {flag}")

    summary = surface.get("coverage_summary", {})
    if summary.get("covered_package_records") != 1:
        raise CityOpsContractError("Document / Handoff operator read surface coverage count drift")
    if summary.get("source_bounded_receipt_blocks_promotion") is not True:
        raise CityOpsContractError("Document / Handoff operator read surface softened receipt/source block")
    if summary.get("custody_scope_blocks_promotion") is not True:
        raise CityOpsContractError("Document / Handoff operator read surface softened custody scope block")
    if summary.get("operator_cards_are_pass_through") is not True:
        raise CityOpsContractError("Document / Handoff operator read surface reinterpreted operator cards")

    cards = surface.get("operator_cards")
    if not isinstance(cards, list) or len(cards) != 6:
        raise CityOpsContractError("Document / Handoff operator read surface cards drift")
    ...
    for required_card in (
        ...
        if required_card not in cards_by_name:
            raise CityOpsContractError("Document / Handoff operator read surface missing required card")
    if cards_by_name["reviewed_output"].get("status") != "package_payload_pass_through_not_customer_copy":
        raise CityOpsContractError("Document / Handoff operator read surface promoted reviewed output")
    ...
    if "raw transcript" in serialized_review and "raw transcript as authority" not in serialized_review:
        raise CityOpsContractError("Document / Handoff operator read surface promoted raw transcript authority")
    if "private person name" not in serialized_review or "not acceptance" not in serialized_review:
        raise CityOpsContractError("Document / Handoff operator read surface lost handoff source bounds")

    checks = surface.get("surface_review_checks")
    if not isinstance(checks, list) or len(checks) != 7:
        raise CityOpsContractError("Document / Handoff operator read surface review checks drift")
    for item in checks:
        if item.get("status") != "passed_for_internal_read_surface_only":
            raise CityOpsContractError("Document / Handoff operator read surface review check status drift")
        if item.get("blocks_promotion_until_later_gate") is not True:
            raise CityOpsContractError("Document / Handoff operator read surface review check stopped blocking promotion")

    _assert_no_exact_location_or_handoff_overclaims(surface)
...
def _assert_claim_boundaries(safe_to_claim: list[str], do_not_claim_yet: list[str]) -> None:
    ...
    if forbidden:
        raise CityOpsContractError(f"Document / Handoff operator read surface has forbidden safe claims: {forbidden}")
    overlap = sorted(safe & blocked)
    if overlap:
        raise CityOpsContractError(f"Document / Handoff operator read surface claim overlap: {overlap}")


def _assert_no_exact_location_or_handoff_overclaims(surface: dict[str, Any]) -> None:
    ...
    for substring in forbidden_substrings:
        if substring in serialized:
            raise CityOpsContractError(
                "Document / Handoff operator read surface leaked exact location or handoff overclaim"
            )
