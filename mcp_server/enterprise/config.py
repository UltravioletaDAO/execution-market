...
class EnterpriseConfig:
    ...
    webhook_events: List[str] = field(default_factory=lambda: [
        'task.created', 'task.completed', 'payment.sent'
    ])
...
PLAN_CONFIGS = {
    ...
    PlanTier.SCALE: {
        ...
        'budget': BudgetConfig(
            ...
            per_task_limit=500.0,
            require_approval_above=250.0
        ),
        'features': {
...
class EnterpriseManager:
    ...
    async def record_spend(self, org_id: str, amount: float):
        ...
        config = self._configs.get(org_id)
        if config:
            usage_pct = self._spend_tracking[org_id]['monthly'] / config.budget.monthly_limit
            if usage_pct >= config.budget.alert_threshold_pct:
                logger.warning(f"Budget alert: {org_id} at {usage_pct:.1%} of monthly limit")
