...
def validate_branding_config(config: BrandingConfig) -> Dict[str, Any]:
    ...
    for field_name in color_fields:
        color = getattr(config.colors, field_name, None)
    ...
    for field_name in url_fields:
        url = getattr(config.assets, field_name, None)
    ...
    if config.custom_domain:
        if "/" in config.custom_domain or ":" in config.custom_domain:
            errors.append(
                f"Custom domain should not include protocol or path: {config.custom_domain}"
            )
