...
def get_escrow_contracts(chain_id: int) -> dict[str, str]:
    ...
    if chain_id not in ESCROW_CONTRACTS:
        ...
        raise ValueError(
            f"No escrow contracts for chain {chain_id}. Supported chains: {supported}"
        )
...
class AdvancedEscrowClient:
    ...
    def _compute_nonce(self, payment_info: PaymentInfo) -> str:
        ...
        final_encoded = encode(
            ...
            [
                self.chain_id,
                Web3.to_checksum_address(self.contracts["escrow"]),
                pi_hash,
            ],
        )
    ...
    def _build_tuple(self, pi: PaymentInfo) -> tuple:
        ...
        if isinstance(pi.salt, int):
            ...
            salt_int = (
                int(pi.salt, 16)
                if pi.salt.startswith("0x")
                else int(pi.salt, 16)
                if all(c in "0123456789abcdefABCDEF" for c in pi.salt)
                else int(pi.salt)
            )
        else:
    ...
    def authorize(self, payment_info: PaymentInfo) -> AuthorizationResult:
        ...
        # Include USDC domain name/version in extra so the Facilitator
        # uses the correct EIP-712 domain for chains where version != "1".
        network_config = get_network_by_chain_id(self.chain_id)
        auth_extra = {
        ...
        if network_config is not None:
            auth_extra["name"] = network_config.usdc_domain_name
            auth_extra["version"] = network_config.usdc_domain_version
    ...
    def _settle_via_facilitator(
        ...
        # Resolve USDC domain name/version for this chain so the Facilitator
        # uses the correct EIP-712 domain when signing the release/refund
        # receiveWithAuthorization (critical for chains like Avalanche where
        # USDC uses domain version "2" instead of the default "1").
        network_config = get_network_by_chain_id(self.chain_id)
        extra = {
            "escrowAddress": self.contracts["escrow"],
            "operatorAddress": self.contracts["operator"],
        ...
        if network_config is not None:
            extra["name"] = network_config.usdc_domain_name
            extra["version"] = network_config.usdc_domain_version
        ...
        try:
            ...
            if result.get("success"):
                ...
                return TransactionResult(
                    success=False,
                    error=result.get(
                        "errorReason", result.get("error", "Unknown error")
                    ),
                )
        except Exception as e:
            return TransactionResult(success=False, error=str(e))
