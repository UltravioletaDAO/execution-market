...
class TestBridgeFilter:
    ...
    def test_event_type_filter(self):
        ...
        assert f.should_bridge(_mock_core_event("task.created")) is True
        assert f.should_bridge(_mock_core_event("task.assigned")) is False
    ...
    def test_category_filter(self):
        f = BridgeFilter(categories={"photo", "delivery"})
        assert f.should_bridge(_mock_core_event(category="photo")) is True
        assert f.should_bridge(_mock_core_event(category="survey")) is False
    ...
    def test_source_exclusion(self):
        f = BridgeFilter(exclude_sources={EventSource.REST_API})
        event = _mock_core_event(source=EventSource.REST_API)
        assert f.should_bridge(event) is False
...
class TestEventHandling:
    ...
    async def test_filtered_event_not_bridged(self):
        swarm_bus = MagicMock(spec=SwarmEventBus)
        f = BridgeFilter(min_bounty_usd=10.0)
        bridge = _make_bridge(swarm_bus=swarm_bus, bridge_filter=f)
        event = _mock_core_event(bounty_usd=1.0)

        await bridge._handle_core_event(event)

        swarm_bus.emit.assert_not_called()
        assert bridge._stats.events_filtered == 1

    @pytest.mark.asyncio
    async def test_unmapped_event_skipped(self):
        swarm_bus = MagicMock(spec=SwarmEventBus)
        bridge = _make_bridge(swarm_bus=swarm_bus)
        event = _mock_core_event("unknown.event.type")

        await bridge._handle_core_event(event)

        swarm_bus.emit.assert_not_called()
        assert bridge._stats.events_skipped == 1
    ...
    @pytest.mark.asyncio
    async def test_default_transform_preserves_payload(self):
        bridge = _make_bridge()
        event = _mock_core_event("task.created")
        event.payload = {"bounty_usd": 5.0, "title": "Test", "category": "photo"}
    ...
    async def test_custom_transform(self):
        def custom(event):
            return {"custom_key": event.task_id}

        swarm_bus = MagicMock(spec=SwarmEventBus)
        bridge = _make_bridge(swarm_bus=swarm_bus, transform_payload=custom)
        event = _mock_core_event("task.created", task_id="t42")

        await bridge._handle_core_event(event)

        call_kwargs = swarm_bus.emit.call_args[1]
        assert call_kwargs["data"]["custom_key"] == "t42"
