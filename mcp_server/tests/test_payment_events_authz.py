...
async def test_enforcement_flag_off_is_legacy(monkeypatch):
    """Kill-switch off → anon allowed, but malformed address still 400."""
    # Anonymous allowed when enforcement off.
    result = await _call(monkeypatch, address=OWNER_WALLET, enforce=False)
    assert "events" in result
    # Format validation is ALWAYS on.
    with pytest.raises(HTTPException) as exc:
        await _call(monkeypatch, address="not-a-valid-wallet-addr-xx", enforce=False)
    assert exc.value.status_code == 400
