...
class TestEffectivePriority:
    ...
    def test_higher_base_priority_wins(self):
        scheduler = make_scheduler()
        high = scheduler.add_task("h", "High", ["x"], priority=TaskPriority.HIGH)
        low = scheduler.add_task("l", "Low", ["x"], priority=TaskPriority.LOW)
...
class TestAgentLoadBalancer:
    ...
    def test_different_agents_independent(self):
        lb = AgentLoadBalancer(max_tasks_per_window=1)
        lb.record_assignment(1)
        assert lb.is_available(1) is False
...
class TestSwarmConditions:
    ...
    def test_budget_headroom(self):
        ...
        assert abs(c.budget_headroom - 0.3) < 0.001
...
class TestTaskManagement:
    def test_add_task(self):
        scheduler = make_scheduler()
        task = scheduler.add_task("t1", "Test", ["photo"], bounty_usd=5.0)
        assert task.task_id == "t1"
        assert scheduler.pending_count == 1

    def test_remove_task(self):
        scheduler = make_scheduler()
        scheduler.add_task("t1", "Test", ["photo"])
...
class TestStatusAndMetrics:
    ...
    def test_decisions_recorded(self):
        scheduler = make_scheduler()
        scheduler.add_task("t1", "T1", ["photo"], bounty_usd=5.0)
        scheduler.compute_schedule()
        decisions = scheduler.get_decisions()
    ...
    def test_urgency_distribution(self):
        scheduler = make_scheduler()
        scheduler.add_task("t1", "Normal", ["x"])
        urgent = datetime.now(timezone.utc) + timedelta(minutes=30)
        scheduler.add_task("t2", "Urgent", ["x"], deadline=urgent)
