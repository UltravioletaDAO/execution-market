...
class TestReplay:
    ...
    def test_replay_unroutable(self):
        engine = ReplayEngine(agents=[])
        task = TaskSnapshot(task_id="t", title="No one can do this")
...
class TestScenarios:
    ...
    def test_scenario_agent_diversity(self, engine, basic_tasks):
        """Multiple different agents → diversity > 0."""
        report = engine.run_scenario("diversity_test", basic_tasks)
        if report.routed_count > 1 and len(report.agent_load) > 1:
    ...
    def test_scenario_avg_confidence(self, engine, basic_tasks):
        report = engine.run_scenario("conf_test", basic_tasks)
        if report.routed_count > 0:
            assert 0 <= report.avg_confidence <= 1.0

    def test_scenario_cost_tracking(self, engine, basic_tasks):
        report = engine.run_scenario("cost_test", basic_tasks)
    ...
    def test_scenario_to_dict(self, engine, basic_tasks):
        report = engine.run_scenario("dict_test", basic_tasks)
        d = report.to_dict()
        assert d["scenario_name"] == "dict_test"
        assert len(d["results"]) == 3
        assert "confidence" in d["results"][0]
...
class TestEngineManagement:
    def test_add_agent(self, engine):
        new_agent = AgentProfile(agent_id="agent_new", skills=["rust"])
        engine.add_agent(new_agent)
        assert "agent_new" in engine.agents

    def test_remove_agent(self, engine):
        assert engine.remove_agent("agent_python") is True
        assert "agent_python" not in engine.agents
...
class TestIO:
    ...
    def test_save_report(self, tmp_path, engine, basic_tasks):
        report = engine.run_scenario("save_test", basic_tasks)
        path = tmp_path / "report.json"
        engine.save_report(report, str(path))

        loaded = json.loads(path.read_text())
        assert loaded["scenario_name"] == "save_test"
        assert loaded["task_count"] == 3

    def test_export_decisions(self, engine, basic_tasks):
        results = [engine.replay_task(t) for t in basic_tasks]
        exported = engine.export_decisions(results)
        assert len(exported) == 3
        assert "task_id" in exported[0]
        assert "confidence" in exported[0]
        assert "simulated_outcome" in exported[0]
