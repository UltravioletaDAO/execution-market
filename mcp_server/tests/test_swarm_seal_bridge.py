...
class TestSealRecommendation:
    ...
    def test_to_dict(self):
        rec = SealRecommendation(
            seal_type="CURIOUS",
            quadrant=SealQuadrant.A2A,
            subject_address="0xABCD",
            evaluator_agent_id="e2",
            score=72,
            confidence=0.85,
            evidence_summary="Diverse work",
            evidence_hash="def456",
            reasoning="Many categories",
        )
        d = rec.to_dict()
        assert d["quadrant"] == "A2A"
        assert d["score"] == 72
        assert d["seal_type"] == "CURIOUS"
...
class TestSealProfile:
    ...
    def test_empty_profile(self):
        p = SealProfile(address="0x1", agent_id="a1")
        assert p.issuable_seals == []
        d = p.to_dict()
...
class TestConfidence:
    ...
    def test_below_minimum(self, bridge):
        assert bridge._compute_confidence(0) == 0.0
        assert bridge._compute_confidence(1) == 0.0
        assert bridge._compute_confidence(2) == 0.0

    def test_at_minimum(self, bridge):
        c = bridge._compute_confidence(3)
        assert c == pytest.approx(0.3, abs=0.01)

    def test_at_high_confidence(self, bridge):
        c = bridge._compute_confidence(10)
        assert c >= 0.4  # 10 tasks is moderate confidence

    def test_at_full_confidence(self, bridge):
        c = bridge._compute_confidence(50)
        assert c == 1.0

    def test_above_full(self, bridge):
        c = bridge._compute_confidence(200)
        assert c == 1.0
...
class TestSkillfulScoring:
    ...
    def test_no_quality_neutral(self, bridge):
        metrics = {"avg_quality": 0, "success_rate": 0.5, "tasks_completed": 10}
        score, _, _ = bridge._score_skillful(metrics)
        assert 30 <= score <= 60  # Neutral range
...
class TestThoroughScoring:
    ...
    def test_no_quality_data(self, bridge):
        metrics = {"avg_quality": 0, "quality_scores": [], "tasks_completed": 5}
...
class TestEngagedScoring:
    ...
    def test_highly_engaged(self, bridge, diverse_worker):
        score, _, _ = bridge._score_engaged(diverse_worker)
        assert score > 65

    def test_inactive_worker(self, bridge):
        metrics = {
            "tasks_completed": 5,
            "categories": {"simple_action": 5},
            "total_revenue_usd": 10,
            "last_active": time.time() - 86400 * 60,  # 60 days ago
        }
        score, _, _ = bridge._score_engaged(metrics)
        # Should lose recency points
        assert score < 50
...
class TestCuriousScoring:
    ...
    def test_single_category(self, bridge):
        metrics = {"categories": {"simple_action": 50}, "tasks_completed": 50}
        score, _, _ = bridge._score_curious(metrics)
        assert score < 30  # Not curious at all

    def test_no_categories(self, bridge):
        metrics = {"categories": {}, "tasks_completed": 0}
        score, _, _ = bridge._score_curious(metrics)
        assert score == 30  # Default
...
class TestFairScoring:
    ...
    def test_no_data(self, bridge):
        metrics = {"categories": {}, "tasks_assigned": 0}
        score, _, _ = bridge._score_fair(metrics)
        assert score == 50


class TestAccurateScoring:
    """Test ACCURATE seal scoring (H2A)."""

    def test_high_completion(self, bridge):
        metrics = {"tasks_completed": 45, "tasks_failed": 3, "tasks_expired": 2}
        score, _, _ = bridge._score_accurate(metrics)
        assert score > 80

    def test_low_completion(self, bridge):
        metrics = {"tasks_completed": 5, "tasks_failed": 20, "tasks_expired": 10}
        score, _, _ = bridge._score_accurate(metrics)
        assert score < 30
...
class TestEthicalScoring:
    ...
    def test_many_expirations(self, bridge):
        metrics = {"tasks_completed": 10, "tasks_failed": 5, "tasks_expired": 15}
        score, _, _ = bridge._score_ethical(metrics)
        assert score < 50
...
class TestEvaluateWorker:
    ...
    def test_scores_clamped_0_100(self, bridge):
        """No score should be below 0 or above 100."""
        extreme_metrics = {
...
class TestIssuanceHistory:
    ...
    def test_record_issuance(self, bridge):
        ...
        bridge.record_issuance(record)
        assert bridge.issuance_count == 1
        history = bridge.get_issuance_history()
        assert len(history) == 1
        assert history[0]["seal_id"] == 42
