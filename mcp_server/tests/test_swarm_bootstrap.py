...
class TestKnownAgents:
    ...
    def test_agent_count(self):
        """24 agents should be registered."""
        assert len(KNOWN_AGENTS) == 24
...
class TestBootstrapResult:
    ...
    def test_to_dict(self):
        result = BootstrapResult(
            ...
            tasks_ingested=189,
            profiles_built=1,
            chains_active=["base", "polygon"],
            total_bounty_usd=18.90,
            bootstrap_ms=150.5,
        )
        d = result.to_dict()
        assert d["agents_registered"] == 24
        assert d["tasks_ingested"] == 189
        assert d["total_bounty_usd"] == 18.9
        assert "base" in d["chains_active"]
...
class TestQuickStart:
    ...
    def test_quick_start_has_agents(self):
        coordinator = SwarmBootstrap.quick_start()
        agents = coordinator.lifecycle.agents
...
class TestFullBootstrap:
    ...
    def test_tag_based_category_scores(self):
        ...
        assert internal is not None
        assert "blockchain" in internal.category_scores
        assert "defi" in internal.category_scores
        assert internal.category_scores["blockchain"] == 30
