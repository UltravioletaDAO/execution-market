...
class TestSealProfile:
    ...
    def test_issuable_seals_filters_by_confidence(self):
        ...
        profile.recommendations = [
            SealRecommendation(
                "SKILLFUL", SealQuadrant.A2H, "0x1", "a", 80, 0.8, "", "", ""
            ),
            SealRecommendation(
                "RELIABLE", SealQuadrant.A2H, "0x1", "a", 70, 0.5, "", "", ""
            ),
        ]
...
class TestSealBridgeConfig:
    ...
    def test_custom_config(self):
        bridge = SealBridge(
            evaluator_agent_id="agent_2106", min_confidence=0.7, gas_per_seal=100000
        )
...
class TestScoreThorough:
    ...
    def test_high_consistent_quality(self):
        score, _, _ = self.bridge._score_thorough(
            {
                "avg_quality": 4.8,
                "quality_scores": [4.8, 4.9, 4.7, 4.8, 4.9],
                "tasks_completed": 5,
            }
        )
        assert score > 80
    ...
    def test_high_but_inconsistent(self):
        consistent, _, _ = self.bridge._score_thorough(
            {"quality_scores": [4.5, 4.5, 4.5, 4.5, 4.5], "tasks_completed": 5}
        )
        inconsistent, _, _ = self.bridge._score_thorough(
            {"quality_scores": [5.0, 1.0, 5.0, 1.0, 5.0], "tasks_completed": 5}
        )
...
class TestScoreCurious:
    ...
    def test_single_category(self):
        score, _, _ = self.bridge._score_curious({"categories": {"delivery": 50}})
...
class TestScoreFair:
    ...
    def test_no_data(self):
        score, _, _ = self.bridge._score_fair({"categories": {}, "tasks_assigned": 0})
        assert score == 50

    def test_even_distribution(self):
        score, _, _ = self.bridge._score_fair(
            {"categories": {"a": 10, "b": 10, "c": 10}, "tasks_assigned": 30}
        )
        assert score > 80

    def test_single_category_concentration(self):
        score, _, _ = self.bridge._score_fair(
            {"categories": {"a": 100}, "tasks_assigned": 100}
        )
...
class TestEvaluateAgentForWorker:
    ...
    def test_below_minimum(self):
        profile = self.bridge.evaluate_agent_for_worker(
            agent_address="0x1", agent_id="a1", metrics={"tasks_assigned": 1}
        )
        assert len(profile.recommendations) == 0
...
class TestFleetEvaluation:
    ...
    def test_address_map_used(self):
        metrics = {"w1": {"tasks_completed": 10, "tasks_failed": 0}}
        profiles = self.bridge.evaluate_fleet(
