"""
Tests for PaymentDispatcher (fase1 and fase2 modes).

Covers:
- Authorize/Release/Refund in fase1 and fase2 modes
- Worker payout consistency (full bounty, no fee deduction)
...
class TestAuthorizePayment:
    ...
    @pytest.mark.asyncio
    async def test_balance_check_only_with_fase1_mode(self):
        """balance_check_only should use fase1 balance check logic."""
...
class TestModeSelection:
    ...
    def test_get_mode_returns_current(self):
        ...
        d = _make_dispatcher("fase1")
        assert d.get_mode() == "fase1"

        d2 = _make_dispatcher("fase2")
        assert d2.get_mode() == "fase2"

    def test_get_info_includes_mode(self):
        """get_info() should include mode and availability."""
        d = _make_dispatcher("fase2")
        info = d.get_info()
        assert info["mode"] == "fase2"
...
class TestCrossModeFallbacks:
    ...
    async def test_dispatcher_exception_handling_authorize(self):
        ...
        d = _make_dispatcher("fase1")
        d._authorize_fase1 = AsyncMock(side_effect=ValueError("Invalid amount"))
    ...
    async def test_dispatcher_exception_handling_release(self):
        ...
        d = _make_dispatcher("fase1")
        d._release_fase1 = AsyncMock(side_effect=KeyError("Missing header"))
    ...
    async def test_dispatcher_exception_handling_refund(self):
        ...
        d = _make_dispatcher("fase1")
        d._refund_fase1 = AsyncMock(side_effect=ConnectionError("DB unavailable"))
