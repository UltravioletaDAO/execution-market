...
def read_sample() -> dict:
    with (ARTIFACT_DIR / RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_retail_reality_internal_sample_output_matches_persisted_artifact():
    sample = build_retail_reality_internal_sample_output()

    assert sample == read_sample()
    assert load_retail_reality_internal_sample_output() == sample
    assert sample["schema"] == RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_SCHEMA
    assert sample["sample_output_id"] == SAMPLE_OUTPUT_ID
    assert sample["scope"] == "internal_admin_retail_reality_sample_output_only"
    assert sample["sample_status"] == (
        "internal_sample_output_landed_not_customer_copy_not_public_not_approved"
    )
    assert RETAIL_REALITY_CUSTOMER_OUTPUT_SCHEMA_GATE_SAFE_CLAIM in sample["safe_to_claim"]
    assert RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in sample["safe_to_claim"]
    assert sample["safe_to_claim"][-1] == RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM
...
def test_sample_consumes_only_customer_output_schema_gate():
    ...
    assert sample["source_schema_gate_file"] == RETAIL_REALITY_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME
    assert contract["consumes_only"] == [RETAIL_REALITY_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME]
    assert contract["source_loader"] == "load_retail_reality_customer_output_schema_gate"
...
def test_sample_populates_only_allowed_schema_fields():
    ...
    assert output["sample_offer"] == "storefront_hours_availability_check"
    ...
    assert "storefront_context_summary" in output["field_values"]
    assert "availability_or_service_state_summary" in output["field_values"]
...
def test_sample_preserves_limitations_privacy_and_non_guarantees():
    output = build_retail_reality_internal_sample_output()["sample_output"]
    ...
    assert reviews["retail_authority_exclusion_review_passed"] is True
    ...
    assert "not publication" in build_retail_reality_internal_sample_output()["next_smallest_proof"]
...
def test_write_retail_reality_internal_sample_output_persists_valid_artifact(tmp_path):
    seed_schema_gate(tmp_path)

    path = write_retail_reality_internal_sample_output(artifact_dir=tmp_path)

    assert path == tmp_path / RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert load_retail_reality_internal_sample_output(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_source_gate_readiness_promotion_fails_closed():
    gate = copy.deepcopy(build_retail_reality_customer_output_schema_gate())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted schema readiness"):
        build_retail_reality_internal_sample_output(schema_gate=gate)
...
def test_loader_fails_closed_on_sample_publication_flip(tmp_path):
    ...
    (tmp_path / RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted sample readiness"):
        load_retail_reality_internal_sample_output(artifact_dir=tmp_path)


def test_loader_fails_closed_on_disallowed_field_value(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_retail_reality_internal_sample_output()
    sample["sample_output"]["field_values"]["inventory_guarantee_claim"] = "blocked"
    (tmp_path / RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_safe_claim(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_retail_reality_internal_sample_output()
    sample["safe_to_claim"].append("internal_sample_inventory_guarantee_ready")
    (tmp_path / RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_retail_claim_text(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_retail_reality_internal_sample_output()
