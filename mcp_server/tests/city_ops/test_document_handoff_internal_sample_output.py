...
from mcp_server.city_ops.document_handoff_internal_sample_output import (
...
def read_sample() -> dict:
    with (ARTIFACT_DIR / DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_document_handoff_internal_sample_output_matches_persisted_artifact():
    sample = build_document_handoff_internal_sample_output()

    assert sample == read_sample()
    assert load_document_handoff_internal_sample_output() == sample
    assert sample["schema"] == DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_SCHEMA
    assert sample["sample_output_id"] == SAMPLE_OUTPUT_ID
    assert sample["scope"] == "internal_admin_document_handoff_sample_output_only"
    assert sample["sample_status"] == (
        "internal_sample_output_landed_not_customer_copy_not_public_not_approved"
    )
    assert DOCUMENT_HANDOFF_CUSTOMER_OUTPUT_SCHEMA_GATE_SAFE_CLAIM in sample[
        "safe_to_claim"
    ]
    assert DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in sample["safe_to_claim"]
    assert sample["safe_to_claim"][-1] == DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM
...
def test_sample_consumes_only_customer_output_schema_gate():
    ...
    assert sample["source_schema_gate_file"] == DOCUMENT_HANDOFF_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME
    assert contract["consumes_only"] == [DOCUMENT_HANDOFF_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME]
    assert contract["source_loader"] == "load_document_handoff_customer_output_schema_gate"
...
def test_sample_populates_only_allowed_schema_fields():
    ...
    assert output["sample_offer"] == "document_handoff_proof_run"
    ...
    assert "handoff_window_summary" in output["field_values"]
...
def test_sample_preserves_limitations_privacy_and_non_guarantees():
    output = build_document_handoff_internal_sample_output()["sample_output"]
    ...
    assert reviews["legal_notarial_and_custody_exclusion_review_passed"] is True
    ...
    assert "not publication" in build_document_handoff_internal_sample_output()["next_smallest_proof"]
...
def test_write_document_handoff_internal_sample_output_persists_valid_artifact(tmp_path):
    seed_schema_gate(tmp_path)

    path = write_document_handoff_internal_sample_output(artifact_dir=tmp_path)

    assert path == tmp_path / DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert load_document_handoff_internal_sample_output(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_source_gate_readiness_promotion_fails_closed():
    gate = copy.deepcopy(build_document_handoff_customer_output_schema_gate())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted schema readiness"):
        build_document_handoff_internal_sample_output(schema_gate=gate)
...
def test_loader_fails_closed_on_sample_publication_flip(tmp_path):
    ...
    (tmp_path / DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted sample readiness"):
        load_document_handoff_internal_sample_output(artifact_dir=tmp_path)


def test_loader_fails_closed_on_disallowed_field_value(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_document_handoff_internal_sample_output()
    sample["sample_output"]["field_values"]["legal_service_claim"] = "blocked"
    (tmp_path / DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_outcome_language(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_document_handoff_internal_sample_output()
    sample["sample_output"]["field_values"]["plain_language_status"] += " Acceptance guaranteed."
    (tmp_path / DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
