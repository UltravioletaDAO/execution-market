...
def read_decision() -> dict:
    with (ARTIFACT_DIR / RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_retail_reality_sample_output_review_decision_matches_persisted_artifact():
    decision = build_retail_reality_sample_output_review_decision()

    assert decision == read_decision()
    assert load_retail_reality_sample_output_review_decision() == decision
    assert decision["schema"] == RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA
    assert decision["decision_id"] == DECISION_ID
    assert decision["scope"] == "internal_admin_retail_reality_sample_output_hold_decision_only"
    assert RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in decision["safe_to_claim"]
    assert RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM in decision["safe_to_claim"]
...
def test_decision_consumes_only_internal_sample_output():
    ...
    assert decision["source_sample_output_file"] == RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert boundary["consumes_only"] == [RETAIL_REALITY_INTERNAL_SAMPLE_OUTPUT_FILENAME]
    ...
    assert boundary["controlled_pilot_allowed"] is False
    ...
    assert boundary["permanent_business_status_claim_allowed"] is False
    assert boundary["inventory_guarantee_allowed"] is False
    assert boundary["brand_compliance_claim_allowed"] is False
    assert boundary["employee_performance_claim_allowed"] is False
    assert boundary["consumer_safety_claim_allowed"] is False
    assert boundary["continuous_availability_monitoring_claim_allowed"] is False
    assert boundary["private_retail_context_release_allowed"] is False
...
def test_decision_keeps_external_product_and_retail_authority_flags_false():
    decision = build_retail_reality_sample_output_review_decision()
...
def test_write_sample_output_review_decision_persists_valid_artifact(tmp_path):
    ...
    path = write_retail_reality_sample_output_review_decision(artifact_dir=tmp_path)

    assert path == tmp_path / RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME
    assert load_retail_reality_sample_output_review_decision(artifact_dir=tmp_path) == json.loads(
...
def test_source_sample_publication_flip_fails_closed():
    ...
    with pytest.raises(CityOpsContractError, match="source promoted sample readiness"):
        build_retail_reality_sample_output_review_decision(sample_output=sample)
...
def test_loader_fails_closed_on_customer_delivery_flip(tmp_path):
    ...
    (tmp_path / RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted readiness"):
        load_retail_reality_sample_output_review_decision(artifact_dir=tmp_path)


def test_loader_fails_closed_on_forbidden_safe_claim(tmp_path):
    decision = build_retail_reality_sample_output_review_decision()
    decision["safe_to_claim"].append("sample_output_inventory_guarantee_ready")
    (tmp_path / RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_retail_authority_language(tmp_path):
    decision = build_retail_reality_sample_output_review_decision()
    decision["operator_instruction"] += " Inventory guaranteed."
    (tmp_path / RETAIL_REALITY_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )
