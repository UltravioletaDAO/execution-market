...
def read_package_record() -> dict:
    with (ARTIFACT_DIR / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)


def test_local_data_collection_internal_package_record_matches_persisted_artifact():
    record = build_local_data_collection_internal_package_record()

    assert record == read_package_record()
    assert load_local_data_collection_internal_package_record() == record
    assert record["schema"] == LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_SCHEMA
    assert record["package_id"] == PACKAGE_ID
    assert record["scope"] == "internal_admin_local_data_collection_package_record_only"
    assert record["offer_id"] == "one_window_count_or_measurement_snapshot"
    assert LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_SAFE_CLAIM in record["safe_to_claim"]
    assert LOCAL_DATA_COLLECTION_LOCAL_REVIEWED_FIXTURE_SAFE_CLAIM in record["safe_to_claim"]
    assert LOCAL_DATA_COLLECTION_FIXTURE_REVIEW_GATE_SAFE_CLAIM in record["safe_to_claim"]
...
def test_package_record_preserves_local_data_evidence_and_reviewed_output_fields():
    record = build_local_data_collection_internal_package_record()
...
def test_package_record_blocks_customer_dataset_analytics_dispatch_reputation_and_worker_doctrine():
    record = build_local_data_collection_internal_package_record()
    package = record["internal_package_record"]
    ...
    assert package["dataset_publication_allowed"] is False
    assert package["analytics_publication_allowed"] is False
    ...
    assert package["statistical_representativeness_claim_allowed"] is False
    assert package["continuous_monitoring_claim_allowed"] is False
    assert package["official_dataset_certification_allowed"] is False
    assert package["exactness_beyond_method_allowed"] is False
    assert package["predictive_analytics_allowed"] is False
...
def test_write_local_data_collection_internal_package_record_persists_valid_artifact(tmp_path):
    path = write_local_data_collection_internal_package_record(artifact_dir=tmp_path)

    assert path == tmp_path / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME
    assert load_local_data_collection_internal_package_record(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_package_record_fails_closed_when_source_fixture_promotes_readiness():
    fixture = copy.deepcopy(build_local_data_collection_local_reviewed_fixture())
    fixture["ladder_boundary"]["promotion_allowed"] = True

    with pytest.raises(CityOpsContractError, match="source promoted readiness"):
        build_local_data_collection_internal_package_record(local_fixture=fixture)


def test_loader_fails_closed_on_readiness_promotion(tmp_path):
    record = build_local_data_collection_internal_package_record()
    record["readiness"]["public_service_catalog_ready"] = True
    (tmp_path / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME).write_text(
        json.dumps(record), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted readiness"):
        load_local_data_collection_internal_package_record(artifact_dir=tmp_path)


def test_loader_fails_closed_on_forbidden_safe_claim(tmp_path):
    record = build_local_data_collection_internal_package_record()
    record["safe_to_claim"].append("local_data_collection_internal_package_dataset_ready")
    (tmp_path / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME).write_text(
        json.dumps(record), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_private_identity_or_exact_location_language(tmp_path):
    record = build_local_data_collection_internal_package_record()
    record["internal_package_record"]["packaged_reviewed_output"][
        "observation_window_summary"
    ] = "exact address retained and private subject identity was copied"
    (tmp_path / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME).write_text(
        json.dumps(record), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_dataset_or_analytics_overclaim_language(tmp_path):
    ...
    ].append("customer dataset authorized and predictive analytics provided")
    (tmp_path / LOCAL_DATA_COLLECTION_INTERNAL_PACKAGE_RECORD_FILENAME).write_text(
        json.dumps(record), encoding="utf-8"
    )
