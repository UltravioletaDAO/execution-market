...
def read_surface() -> dict:
    with (ARTIFACT_DIR / RETAIL_REALITY_OPERATOR_READ_SURFACE_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_retail_reality_operator_read_surface_matches_persisted_artifact():
    surface = build_retail_reality_operator_read_surface()

    assert surface == read_surface()
    assert load_retail_reality_operator_read_surface() == surface
    assert surface["schema"] == RETAIL_REALITY_OPERATOR_READ_SURFACE_SCHEMA
    assert surface["surface_id"] == SURFACE_ID
    assert surface["scope"] == "internal_admin_retail_reality_read_surface_only"
    assert surface["offer_id"] == "storefront_hours_availability_check"
    assert RETAIL_REALITY_OPERATOR_READ_SURFACE_SAFE_CLAIM in surface["safe_to_claim"]
    assert RETAIL_REALITY_INTERNAL_PACKAGE_RECORD_SAFE_CLAIM in surface["safe_to_claim"]
...
def test_read_surface_consumes_only_internal_package_record_and_preserves_digest():
    package_record = build_retail_reality_internal_package_record()
    surface = build_retail_reality_operator_read_surface(package_record=package_record)
    ...
    assert surface["derived_from"]["reads_inventory_systems"] is False
...
def test_safe_and_blocked_claims_are_adjacent_in_persisted_surface():
    path = ARTIFACT_DIR / RETAIL_REALITY_OPERATOR_READ_SURFACE_FILENAME
...
def test_read_surface_blocks_external_product_retail_and_worker_claims():
    surface = build_retail_reality_operator_read_surface()
...
def test_write_retail_reality_operator_read_surface_persists_valid_artifact(tmp_path):
    path = write_retail_reality_operator_read_surface(artifact_dir=tmp_path)

    assert path == tmp_path / RETAIL_REALITY_OPERATOR_READ_SURFACE_FILENAME
    assert load_retail_reality_operator_read_surface(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_read_surface_fails_closed_when_source_package_promotes_readiness():
    package = copy.deepcopy(build_retail_reality_internal_package_record())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted readiness"):
        build_retail_reality_operator_read_surface(package_record=package)


def test_read_surface_fails_closed_when_source_package_promotes_retail_claim():
    package = copy.deepcopy(build_retail_reality_internal_package_record())
    package["internal_package_record"]["inventory_guarantee_allowed"] = True
...
def test_loader_fails_closed_on_public_access_upgrade(tmp_path):
    ...
    with pytest.raises(CityOpsContractError, match="access_policy overclaims"):
        load_retail_reality_operator_read_surface(artifact_dir=tmp_path)
...
def test_loader_fails_closed_on_retail_overclaim_language(tmp_path):
    surface = build_retail_reality_operator_read_surface()
    surface["operator_instruction"] += " inventory guarantee provided"
    (tmp_path / RETAIL_REALITY_OPERATOR_READ_SURFACE_FILENAME).write_text(
        json.dumps(surface), encoding="utf-8"
    )
