...
from mcp_server.city_ops.compliance_desk_internal_sample_output import (
...
def read_sample() -> dict:
    with (ARTIFACT_DIR / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_compliance_desk_internal_sample_output_matches_persisted_artifact():
    sample = build_compliance_desk_internal_sample_output()

    assert sample == read_sample()
    assert load_compliance_desk_internal_sample_output() == sample
    assert sample["schema"] == COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_SCHEMA
    assert sample["sample_output_id"] == SAMPLE_OUTPUT_ID
    assert sample["scope"] == "internal_admin_compliance_desk_sample_output_only"
    assert sample["sample_status"] == (
        "internal_sample_output_landed_not_customer_copy_not_public_not_approved"
    )
    assert COMPLIANCE_DESK_CUSTOMER_OUTPUT_SCHEMA_GATE_SAFE_CLAIM in sample[
        "safe_to_claim"
    ]
    assert COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in sample["safe_to_claim"]
    assert sample["safe_to_claim"][-1] == COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM
...
def test_sample_consumes_only_customer_output_schema_gate():
    ...
    assert sample["source_schema_gate_file"] == COMPLIANCE_DESK_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME
    assert contract["consumes_only"] == [COMPLIANCE_DESK_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME]
    assert contract["source_loader"] == "load_compliance_desk_customer_output_schema_gate"
...
def test_sample_populates_only_allowed_schema_fields():
    ...
    assert output["sample_offer"] == "visible_posting_notice_compliance_snapshot"
    ...
    assert "limitations" in output["field_values"]
    for forbidden in FORBIDDEN_CUSTOMER_OUTPUT_FIELDS:
        assert forbidden not in output["field_values"]


def test_sample_preserves_limitations_privacy_and_non_guarantees():
    output = build_compliance_desk_internal_sample_output()["sample_output"]
    ...
    assert reviews["legal_advice_exclusion_review_passed"] is True
    ...
    assert "not customer-ready copy" in " ".join(values["limitations"])
    assert "separate explicit hold/approval decision" in " ".join(values["limitations"])
    assert "Privacy-sensitive details" in values["privacy_redaction_notice"]
    assert "not publication" in build_compliance_desk_internal_sample_output()["next_smallest_proof"]
...
def test_write_compliance_desk_internal_sample_output_persists_valid_artifact(tmp_path):
    seed_schema_gate(tmp_path)

    path = write_compliance_desk_internal_sample_output(artifact_dir=tmp_path)

    assert path == tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert load_compliance_desk_internal_sample_output(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_source_gate_readiness_promotion_fails_closed():
    gate = copy.deepcopy(build_compliance_desk_customer_output_schema_gate())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted schema readiness"):
        build_compliance_desk_internal_sample_output(schema_gate=gate)
...
def test_loader_fails_closed_on_sample_publication_flip(tmp_path):
    ...
    (tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted sample readiness"):
        load_compliance_desk_internal_sample_output(artifact_dir=tmp_path)


def test_loader_fails_closed_on_disallowed_field_value(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_compliance_desk_internal_sample_output()
    sample["sample_output"]["field_values"]["legal_sufficiency_claim"] = "blocked"
    (tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_missing_review_gate(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_compliance_desk_internal_sample_output()
    sample["sample_output"]["separate_reviews"]["non_guarantee_language_review_passed"] = False
    (tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_safe_claim(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_compliance_desk_internal_sample_output()
    sample["safe_to_claim"].append("internal_sample_publication_ready")
    (tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_outcome_language(tmp_path):
    seed_schema_gate(tmp_path)
    sample = build_compliance_desk_internal_sample_output()
    sample["sample_output"]["field_values"]["plain_language_status"] += " Guaranteed compliance."
    (tmp_path / COMPLIANCE_DESK_INTERNAL_SAMPLE_OUTPUT_FILENAME).write_text(
        json.dumps(sample), encoding="utf-8"
    )
