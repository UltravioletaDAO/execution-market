...
def read_decision() -> dict:
    with (ARTIFACT_DIR / LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_local_data_collection_sample_output_review_decision_matches_persisted_artifact():
    decision = build_local_data_collection_sample_output_review_decision()

    assert decision == read_decision()
    assert load_local_data_collection_sample_output_review_decision() == decision
    assert decision["schema"] == LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA
    assert decision["decision_id"] == DECISION_ID
    assert decision["scope"] == "internal_admin_local_data_collection_sample_output_hold_decision_only"
    assert LOCAL_DATA_COLLECTION_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in decision["safe_to_claim"]
    assert LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_SAFE_CLAIM in decision["safe_to_claim"]
...
def test_decision_records_explicit_hold_not_approval():
    ...
    assert decision["customer_visible_catalog_ready"] is False
...
def test_decision_consumes_only_internal_sample_output():
    ...
    assert decision["source_sample_output_file"] == LOCAL_DATA_COLLECTION_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert boundary["consumes_only"] == [LOCAL_DATA_COLLECTION_INTERNAL_SAMPLE_OUTPUT_FILENAME]
...
def test_decision_keeps_external_product_and_data_claim_flags_false():
    decision = build_local_data_collection_sample_output_review_decision()
...
def test_write_sample_output_review_decision_persists_valid_artifact(tmp_path):
    ...
    path = write_local_data_collection_sample_output_review_decision(artifact_dir=tmp_path)

    assert path == tmp_path / LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME
    assert load_local_data_collection_sample_output_review_decision(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_source_sample_publication_flip_fails_closed():
    sample = copy.deepcopy(build_local_data_collection_internal_sample_output())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted sample readiness"):
        build_local_data_collection_sample_output_review_decision(sample_output=sample)
...
def test_loader_fails_closed_on_customer_delivery_flip(tmp_path):
    ...
    (tmp_path / LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )

    with pytest.raises(CityOpsContractError, match="promoted readiness"):
        load_local_data_collection_sample_output_review_decision(artifact_dir=tmp_path)


def test_loader_fails_closed_on_forbidden_safe_claim(tmp_path):
    decision = build_local_data_collection_sample_output_review_decision()
    decision["safe_to_claim"].append("sample_output_dataset_ready")
    (tmp_path / LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )
...
def test_loader_fails_closed_on_forbidden_data_claim_language(tmp_path):
    decision = build_local_data_collection_sample_output_review_decision()
    decision["operator_instruction"] += " Representative dataset confirmed."
    (tmp_path / LOCAL_DATA_COLLECTION_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )
