...
def read_gate() -> dict:
    with (ARTIFACT_DIR / INCIDENT_VERIFICATION_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)


def test_incident_verification_customer_output_schema_gate_matches_persisted_artifact():
    gate = build_incident_verification_customer_output_schema_gate()

    assert gate == read_gate()
    assert load_incident_verification_customer_output_schema_gate() == gate
    assert gate["schema"] == INCIDENT_VERIFICATION_CUSTOMER_OUTPUT_SCHEMA_GATE_SCHEMA
    ...
    assert INCIDENT_VERIFICATION_CUSTOMER_OUTPUT_SCHEMA_GATE_SAFE_CLAIM in gate["safe_to_claim"]
    assert INCIDENT_VERIFICATION_OPERATOR_READ_SURFACE_SAFE_CLAIM in gate["safe_to_claim"]
...
def test_schema_gate_consumes_only_operator_read_surface_and_builder():
    ...
    assert gate["source_surface_file"] == INCIDENT_VERIFICATION_OPERATOR_READ_SURFACE_FILENAME
    assert contract["consumes_only"] == [INCIDENT_VERIFICATION_OPERATOR_READ_SURFACE_FILENAME]
    assert contract["source_builder"] == "build_incident_verification_operator_read_surface"
...
def test_schema_gate_blocks_external_product_and_incident_outcome_claims():
    gate = build_incident_verification_customer_output_schema_gate()
    ...
    assert gate["safe_to_claim"][-1] == INCIDENT_VERIFICATION_CUSTOMER_OUTPUT_SCHEMA_GATE_SAFE_CLAIM
...
def test_write_incident_verification_customer_output_schema_gate_persists_valid_artifact(tmp_path):
    source = build_incident_verification_operator_read_surface()
    (tmp_path / INCIDENT_VERIFICATION_OPERATOR_READ_SURFACE_FILENAME).write_text(
        json.dumps(source), encoding="utf-8"
    )

    path = write_incident_verification_customer_output_schema_gate(artifact_dir=tmp_path)

    assert path == tmp_path / INCIDENT_VERIFICATION_CUSTOMER_OUTPUT_SCHEMA_GATE_FILENAME
    assert load_incident_verification_customer_output_schema_gate(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_schema_gate_fails_closed_when_source_surface_promotes_public_access():
    surface = copy.deepcopy(build_incident_verification_operator_read_surface())
    surface["access_policy"]["public_route_registered"] = True

    with pytest.raises(CityOpsContractError, match="source access overclaims"):
        build_incident_verification_customer_output_schema_gate(operator_surface=surface)


def test_loader_fails_closed_on_customer_copy_created(tmp_path):
    source = build_incident_verification_operator_read_surface()
    gate = build_incident_verification_customer_output_schema_gate(operator_surface=source)
    ...
    with pytest.raises(CityOpsContractError, match="promoted schema readiness"):
        load_incident_verification_customer_output_schema_gate(artifact_dir=tmp_path)
...
def test_loader_fails_closed_on_incident_overclaim_language(tmp_path):
    source = build_incident_verification_operator_read_surface()
    gate = build_incident_verification_customer_output_schema_gate(operator_surface=source)
    gate["schema_review"]["required_boundary_notes"].append(
