...
def read_decision() -> dict:
    with (ARTIFACT_DIR / DOCUMENT_HANDOFF_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).open(
        "r", encoding="utf-8"
    ) as fh:
        return json.load(fh)
...
def test_document_handoff_sample_output_review_decision_matches_persisted_artifact():
    decision = build_document_handoff_sample_output_review_decision()

    assert decision == read_decision()
    assert load_document_handoff_sample_output_review_decision() == decision
    assert decision["schema"] == DOCUMENT_HANDOFF_SAMPLE_OUTPUT_REVIEW_DECISION_SCHEMA
    assert decision["decision_id"] == DECISION_ID
    assert decision["scope"] == "internal_admin_document_handoff_sample_output_hold_decision_only"
    assert DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_SAFE_CLAIM in decision["safe_to_claim"]
...
def test_decision_consumes_only_internal_sample_output():
    ...
    assert decision["source_sample_output_file"] == DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME
    assert boundary["consumes_only"] == [DOCUMENT_HANDOFF_INTERNAL_SAMPLE_OUTPUT_FILENAME]
...
def test_write_sample_output_review_decision_persists_valid_artifact(tmp_path):
    ...
    path = write_document_handoff_sample_output_review_decision(artifact_dir=tmp_path)

    assert path == tmp_path / DOCUMENT_HANDOFF_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME
    assert load_document_handoff_sample_output_review_decision(artifact_dir=tmp_path) == json.loads(
        path.read_text(encoding="utf-8")
    )


def test_source_sample_publication_flip_fails_closed():
    sample = copy.deepcopy(build_document_handoff_internal_sample_output())
    ...
    with pytest.raises(CityOpsContractError, match="source promoted sample readiness"):
        build_document_handoff_sample_output_review_decision(sample_output=sample)
...
def test_loader_fails_closed_on_customer_delivery_flip(tmp_path):
    ...
    with pytest.raises(CityOpsContractError, match="promoted readiness"):
        load_document_handoff_sample_output_review_decision(artifact_dir=tmp_path)
...
def test_loader_fails_closed_on_forbidden_authority_language(tmp_path):
    decision = build_document_handoff_sample_output_review_decision()
    decision["operator_instruction"] += " Acceptance guaranteed."
    (tmp_path / DOCUMENT_HANDOFF_SAMPLE_OUTPUT_REVIEW_DECISION_FILENAME).write_text(
        json.dumps(decision), encoding="utf-8"
    )
