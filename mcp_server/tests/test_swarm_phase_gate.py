...
class TestPhaseEvaluation:
    ...
    def test_can_advance_blocker_fails(self):
        ev = PhaseEvaluation(
            ...
            target_phase=Phase.PASSIVE,
            gates=[
                GateCheck(name="a", description="A passes", passed=True),
                GateCheck(name="b", description="B fails", passed=False),
            ],
        )
        assert ev.can_advance is False
        assert "B fails" in ev.blockers
...
class TestPhaseGateEmergencyStop:
    def test_emergency_stop(self):
        gate = PhaseGate(initial_phase=Phase.SEMI_AUTO)
        gate.emergency_stop("API down")

        assert gate.phase == Phase.EMERGENCY
        assert gate.mode == "disabled"
        assert len(gate.history) == 1
        assert "EMERGENCY" in gate.history[0].reason
...
class TestPhaseTransition:
    def test_to_dict(self):
        ...
        d = t.to_dict()
        assert "Pre-Flight" in d["from"]
        assert "Passive" in d["to"]
