...
class TestSkillGapAnalysis:
    ...
    def test_empty_agents(self):
        planner = CapacityPlanner()
        tasks = _tasks([("photo", None), ("photo", None), ("video", None)])
        result = planner.analyze_skill_gaps([], tasks)
    ...
    def test_title_based_skill_extraction(self):
        planner = CapacityPlanner()
        tasks = [{"title": "Take a photo of the storefront", "category": ""}]
        agents = []
...
class TestConcentrationRisk:
    def test_no_tasks(self):
        planner = CapacityPlanner()
        result = planner.concentration_risk(_agents(3), [])
...
class TestRecruitmentPlan:
    ...
    def test_sorted_by_urgency(self):
        planner = CapacityPlanner()
        agents = [{"skills": {"photo": 0.8}}]
        tasks = _tasks([("video", None)] * 5 + [("photo", None)])
    ...
    def test_empty_inputs(self):
        planner = CapacityPlanner()
        # Even with empty inputs, capacity forecast shows critical shortage
        # (0 agents for default 10 projected tasks/day)
        plan = planner.recruitment_plan([], [], projected_daily_tasks=0.0)
        assert plan["critical_count"] == 0
