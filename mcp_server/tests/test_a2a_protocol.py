...
class TestA2AModels:
    ...
    def test_parse_part_file(self):
        ...
        part = parse_part({"kind": "file", "mimeType": "image/jpeg", "uri": "https://x.com/img.jpg"})
...
class TestA2ATaskManager:
    ...
    def test_em_task_to_a2a_with_history(self):
        ...
        em_task = {
            ...
            "created_at": "2026-02-12T04:00:00Z",
            "status_history": [
                {"type": "status_change", "message": "Worker assigned", "timestamp": "2026-02-12T04:10:00Z"},
            ],
        }
...
class TestA2AJSONRPCRouter:
    ...
    def test_batch_request(self, client):
        with patch("a2a.jsonrpc_router._dispatch") as mock_dispatch:
            ...
            resp = client.post("/a2a/v1", json=[
                {"jsonrpc": "2.0", "id": 1, "method": "tasks/get", "params": {"id": "a"}},
                {"jsonrpc": "2.0", "id": 2, "method": "tasks/get", "params": {"id": "b"}},
            ])
    ...
    def test_auth_extraction_api_key(self, client):
        ...
        with patch("a2a.task_manager.db", create=True) as mock_db:
            mock_db.get_task.return_value = {
                "id": "t1", "title": "Auth test", "status": "published",
                "bounty_usd": 1.0, "created_at": "2026-02-12T04:00:00Z",
            }
...
class TestAgentCard:
    ...
    def test_agent_card_url_normalization(self):
        ...
        assert _normalize_base_url("http://remote.host.com") == "https://remote.host.com"
