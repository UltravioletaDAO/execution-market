...
class ProgressionSystem:
    ...
    ACHIEVEMENTS = [
        ...
        Achievement(
            "no_rejections",
            "Flawless",
            "50 tasks without rejection",
            AchievementCategory.QUALITY,
            "diamond",
            100,
            {"consecutive_approved": 50},
            rarity="rare",
        ),
        Achievement(
            "photo_pro",
            "Photo Pro",
            "All photos rated excellent for 20 tasks",
            AchievementCategory.QUALITY,
            "camera",
            50,
            {"excellent_photos": 20},
        ...
        Achievement(
            "reliable",
            "Old Reliable",
            "90% completion rate over 100 tasks",
            AchievementCategory.CONSISTENCY,
            "lock",
            75,
            {"completion_rate": 0.9, "min_tasks": 100},
        ),
        # Social
        Achievement(
            "referral_1",
    ...
    def get_level_for_tasks(self, task_count: int) -> WorkerLevel:
        ...
        for level_config in sorted(
            self.LEVELS.values(), key=lambda x: x.min_tasks, reverse=True
        ):
    ...
    def award_xp(
        self, worker_id: str, amount: int, reason: str
    ...
    def _check_achievements(self, progress: WorkerProgress) -> List[Achievement]:
        ...
        for achievement in self.ACHIEVEMENTS:
            ...
            if met:
                ...
                logger.info(
                    f"Worker {progress.worker_id} unlocked achievement: {achievement.name}"
                )
    ...
    def get_progress(self, worker_id: str) -> Dict[str, Any]:
        ...
        if level_config.max_tasks == -1:
            ...
            progress_percent = min(
                100, int((tasks_done_in_level / tasks_in_level) * 100)
            )
    ...
    def get_leaderboard(
        self, limit: int = 10, metric: str = "xp"
    ) -> List[Dict[str, Any]]:
        """
        ...
        if metric == "xp":
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: p.xp, reverse=True
            )
        elif metric == "tasks":
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: p.total_tasks, reverse=True
            )
        elif metric == "earnings":
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: p.total_earnings, reverse=True
            )
        elif metric == "achievements":
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: len(p.achievements), reverse=True
            )
        elif metric == "streak":
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: p.current_streak, reverse=True
            )
        else:
            sorted_workers = sorted(
                self._progress.values(), key=lambda p: p.xp, reverse=True
