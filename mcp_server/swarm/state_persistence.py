...
class PersistedState:
    ...
    # Agent state
    agent_reputations: dict = field(default_factory=dict)  # agent_id → {on_chain, internal}
    
    # Retry backoffs
    retry_backoffs: dict = field(default_factory=dict)  # task_id → {next_retry_at, attempt}
...
class RetryBackoff:
    ...
    def __init__(self):
        self._backoffs: dict[str, dict] = {}  # task_id → {next_retry_at, attempt, delay}
    ...
    def pending_count(self) -> int:
        ...
        return sum(
            1 for entry in self._backoffs.values()
