...
def _em_task_to_a2a(
    ...
    return A2ATask(
        ...
        status=A2ATaskStatus(
            state=a2a_state,
            message=status_msg,
            timestamp=em_task.get("updated_at")
            or em_task.get("created_at")
            or now_iso(),
        ),
...
def _build_history(em_task: Dict[str, Any]) -> List[Message]:
    ...
    history.append(
        Message(
            ...
            parts=[
                TextPart(
                    text=f"Task created: {em_task.get('title', '')}. "
...
class A2ATaskManager:
    ...
    async def cancel_task(self, task_id: str) -> Optional[A2ATask]:
        ...
        try:
            ...
            if self.agent_id and task.get("agent_id") != self.agent_id:
                logger.warning(
                    f"Agent {self.agent_id} tried to cancel task {task_id} they don't own"
                )
                return None
            ...
            if task.get("status") not in cancellable_states:
                logger.info(
                    f"Task {task_id} in state {task.get('status')} not cancellable"
                )
                return None
        ...
        try:
            ...
            for part in message.parts:
                if isinstance(part, TextPart):
                    ...
                    elif text_lower.startswith("reject") or text_lower.startswith(
                        "dispute"
                    ):
                        db.update_task(
                            task_id,
