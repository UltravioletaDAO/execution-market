...
async def _extract_agent_id(request: Request) -> Optional[str]:
    ...
    if api_key:
        # In production, resolve API key to agent_id via auth module
        try:
            from api.auth import verify_api_key_optional, APIKeyData
            # Simplified: use key prefix as agent_id
...
async def a2a_jsonrpc_endpoint(request: Request):
    ...
    if isinstance(body, list):
        ...
        results = await asyncio.gather(
            *[_dispatch(req, agent_id) for req in body]
        )
