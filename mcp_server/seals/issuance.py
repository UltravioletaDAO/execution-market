...
class SealIssuanceService:
    ...
    def check_eligibility(
        ...
        if requirement.min_tasks is not None:
            if requirement.task_category:
                ...
                category_tasks = stats.tasks_by_category.get(requirement.task_category, 0)
        ...
        if requirement.seal_type == BehaviorSealType.NEVER_CANCELLED.value:
            if stats.cancellation_count > 0:
                missing.append(
                    f"Need 0 cancellations, have {stats.cancellation_count}"
                )
        ...
        is_eligible = len(missing) == 0 and (
            not requirement.requires_test and
            not requirement.requires_portfolio
        )
    ...
    def check_all_eligibility(
        self,
        stats: WorkerStats,
        category: Optional[SealCategory] = None
    ...
    def _calculate_progress(
        ...
        if requirement.min_earnings_usd is not None:
            return min(99, int(stats.total_earnings_usd / requirement.min_earnings_usd * 100))
    ...
    async def issue_seal(
        ...
        try:
            ...
            if requirement and requirement.expires_after_days:
                expires_at = datetime.now(UTC) + timedelta(days=requirement.expires_after_days)
    ...
    async def check_and_issue_automatic(
        ...
        for eligibility in eligibility_results:
            ...
            # Check if already has seal
            if await self.registry.has_seal(stats.wallet_address, eligibility.seal_type):
                continue

            if dry_run:
                results.append(IssuanceResult(
                    success=True,
            ...
            try:
                ...
                for issuance in issuances:
                    ...
                    if wallet not in results:
                        results[wallet] = []
                    results[wallet].append(IssuanceResult(
                        success=True,
