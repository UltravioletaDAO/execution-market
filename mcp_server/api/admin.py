...
async def log_admin_action(
    ...
    """Insert a row into admin_actions_log. Non-critical — failures are logged but swallowed."""
    try:
        supabase.table("admin_actions_log").insert(
            {
                "actor_id": actor_id,
        ...
        logger.warning("Failed to log admin action %s: %s", action_type, e)
...
# verify_admin_key is imported from admin_auth.py so /api/v1/admin and
# /internal/admin routes share the exact same boundary.
...
# =============================================================================
# CONFIG BULK EXPORT / IMPORT
# IMPORTANT: These MUST be defined BEFORE /config/{key} to avoid
# FastAPI matching "export"/"import" as a {key} path parameter.
# =============================================================================


class ConfigExportResponse(BaseModel):
    """Bulk export of platform configuration as flat key-value pairs."""

    configs: Dict[str, Any] = Field(..., description="Flat map of config key to value")
    exported_at: str = Field(..., description="ISO 8601 export timestamp")
    total: int = Field(..., description="Number of config entries exported")


class ConfigImportChange(BaseModel):
    """A single change detected during config import."""
...
class ConfigImportRequest(BaseModel):
    """Request body for bulk config import."""

    configs: Dict[str, Any] = Field(
        ..., description="Flat map of config key to new value"
    )
    reason: Optional[str] = Field(
        None, description="Reason for bulk change (recorded in audit log)"
    )
    dry_run: bool = Field(
        False, description="If true, preview changes without committing"
    )


class ConfigImportResponse(BaseModel):
    """Response from bulk config import."""

    updated: int = Field(..., description="Number of keys updated")
    skipped: int = Field(..., description="Number of keys unchanged or missing")
    changes: List[ConfigImportChange] = Field(..., description="Per-key change details")
...
@router.get(
    ...
    summary="Export All Configuration",
    description=(
        "Export all platform configuration as a flat JSON object. "
        "Optionally filter by category using repeated ?category= query params."
    ),
    responses={
        200: {"description": "Exported configuration"},
        401: {"description": "Unauthorized"},
        503: {"description": "Configuration system not available"},
    },
)
async def export_config(
    category: Optional[List[str]] = Query(
        None,
        description="Filter by category (repeatable, e.g. ?category=fees&category=limits)",
    ),
    admin: dict = Depends(verify_admin_key),
) -> ConfigExportResponse:
    """Export all config as a flat key-value map, optionally filtered by category."""
    if not CONFIG_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="Configuration system not available"
        )
    ...
    try:
        supabase = db.get_supabase_client()
        query = supabase.table("platform_config").select("key, value, category")
        result = query.execute()

        configs: Dict[str, Any] = {}
        for row in result.data or []:
            if category and row.get("category") not in category:
                continue
            value = row["value"]
            if isinstance(value, Decimal):
                value = float(value)
        ...
        # Fall back to PlatformConfig defaults if DB empty
        if not result.data and CONFIG_AVAILABLE:
            valid_categories = set(category) if category else None
            for cat in ConfigCategory:
                if valid_categories and cat.value not in valid_categories:
                    continue
                cat_configs = await PlatformConfig.get_all_by_category(cat)
                for key, value in cat_configs.items():
                    if isinstance(value, Decimal):
                        value = float(value)
                    configs[key] = value

        logger.info(
            "SECURITY_AUDIT action=admin.config_export actor=%s categories=%s total=%d",
            admin.get("actor_id"),
            category,
            len(configs),
        )
...
@router.post(
    ...
    description=(
        "Bulk update platform configuration from a flat key-value map. "
        "Use dry_run=true to preview changes without committing. "
        "Only existing keys are updated; unknown keys are skipped with an error."
    ),
    responses={
        200: {"description": "Import results with per-key change details"},
        401: {"description": "Unauthorized"},
        503: {"description": "Configuration system not available"},
    },
)
async def import_config(
    ...
    """Bulk update config. Supports dry_run to preview changes."""
    if not CONFIG_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="Configuration system not available"
        )
    ...
    try:
        ...
        # Fetch all current config in one query
        current_result = (
            supabase.table("platform_config").select("key, value").execute()
        )
        ...
        for key, new_value in request.configs.items():
            if key not in current_map:
                errors.append(f"Key '{key}' not found in platform_config")
                changes.append(
                    ConfigImportChange(
                        key=key,
                        old=None,
            ...
            # Normalize for comparison (handle float vs int, etc.)
            old_cmp = (
                float(old_value)
            ...
            if not request.dry_run:
                try:
                    supabase.table("platform_config").update(
                        {
                            "value": new_value,
        ...
        logger.info(
            "SECURITY_AUDIT action=admin.config_import actor=%s dry_run=%s updated=%d skipped=%d errors=%d reason=%s",
            admin.get("actor_id"),
            request.dry_run,
            updated,
            skipped,
...
# =============================================================================
# ADMIN ACTIONS LOG ENDPOINT
# =============================================================================


class AdminActionEntry(BaseModel):
    """Single entry in the admin actions log."""

    id: str = Field(..., description="Unique action entry ID")
    actor_id: str = Field(..., description="Admin actor who performed the action")
    action_type: str = Field(
        ..., description="Type of action (cancel_task, suspend_user, etc.)"
    )
    target_type: Optional[str] = Field(
        None, description="Target entity type (task, user, payment)"
    )
    ...
    result: str = Field(..., description="Action result (success or error)")
    created_at: datetime = Field(..., description="Timestamp of the action (ISO 8601)")


class AdminActionsLogResponse(BaseModel):
    """Paginated list of admin action log entries."""

    entries: List[AdminActionEntry] = Field(
...
@router.get(
    "/actions/log",
    response_model=AdminActionsLogResponse,
    summary="Get Admin Actions Log",
    description="Retrieve the audit trail of admin actions (cancel task, suspend user, sweep fees, retry payment, update task). Supports filtering by action type.",
    responses={
        200: {"description": "Paginated admin action log entries"},
        401: {"description": "Unauthorized"},
    },
)
async def get_admin_actions_log(
    ...
    try:
        supabase = db.get_supabase_client()
        query = supabase.table("admin_actions_log").select("*", count="exact")
        ...
        logger.error(f"Error getting admin actions log: {e}")
        return AdminActionsLogResponse(entries=[], count=0, offset=offset)
...
async def update_task(
    ...
    try:
        ...
        await log_admin_action(
            supabase,
            actor_id=admin.get("actor_id", "system"),
            action_type="update_task",
            target_type="task",
            target_id=task_id,
            details={"fields_updated": list(filtered_updates.keys())},
        )
...
async def cancel_task(
    ...
    try:
        ...
        await log_admin_action(
            ...
            target_type="task",
            target_id=task_id,
            details={"reason": reason, "previous_status": task.data["status"]},
        )
    ...
    try:
        ...
        try:
            ...
            await log_admin_action(
                supa,
                actor_id=admin.get("actor_id", "system"),
                action_type="sweep_fees",
                target_type="treasury",
                target_id=None,
                details={"network": network, "token": token, "result": result},
            )
        except Exception:
            pass  # Non-critical
    ...
    try:
        ...
        if success:
            ...
            await log_admin_action(
                ...
                target_type="submission",
                target_id=submission_id,
                details={"task_id": submission.get("task_id")},
                result="error",
            )
