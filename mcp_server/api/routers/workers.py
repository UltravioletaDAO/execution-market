...
async def get_submission_detail(
    ...
    # Fail-closed: an anonymous caller is rejected (was "log but allow", which
    # leaked evidence S3 keys to anon callers regardless of EM_REQUIRE_WORKER_AUTH).
...
async def apply_to_task(
    ...
    """
    # FIX-P1-01: fail-closed identity resolution. Allows the publishing agent
    # (ERC-8128) to act for its assigned worker (H2H) without a Supabase JWT.
    ...
    # Shared utility used by both REST API and MCP tools.
    ...
    _wid_allowed, _wid_error = await check_world_id_eligibility(
        executor_id, task_bounty, db_client=db.get_client()
    )
    if not _wid_allowed:
...
async def submit_work(
    ...
    """
    # FIX-P1-01: fail-closed identity resolution + H2H agent path.
    executor_id = await resolve_worker_identity(
        raw_request, worker_auth, request.executor_id, task_id=task_id
    )
    try:
...
async def get_payment_events(
    ...
    AuthZ (FIX-P1-03): caller must either (a) be the worker who owns *address*
    (Supabase JWT), or (b) present a valid internal API key (X-API-Key/Bearer).
    Anonymous and cross-wallet requests are rejected. The address is strictly
    format-validated to prevent PostgREST filter injection.
    """
