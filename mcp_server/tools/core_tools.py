...
def register_core_tools(
    ...
    async def em_publish_task(
        ...
        try:
            ...
            # for ERC-8128 callers. With enforcement off (EM_MCP_AUTH_ENABLED
            # unset/false — staged rollout) this falls back to params.agent_id.
            try:
...
The task is now visible to human executors. Use `em_get_task` with the task ID to monitor progress, or `em_check_submission` when a human submits evidence."""
    ...
    async def em_approve_submission(
        ...
        try:
            ...
            # With enforcement off (EM_MCP_AUTH_ENABLED unset/false) this returns
            # the lowercased params.agent_id — byte-identical to the legacy
            # case-insensitive ownership check below.
            try:
...
- **Transaction**: `{tx_hash_str}`"""
    ...
    async def em_cancel_task(
        ...
        try:
            # FIX-P0-01: authorize against the verified caller, never the body.
            # Enforcement off → returns lowercased params.agent_id (byte-identical
            # to the legacy case-insensitive check below).
            try:
                caller_agent_id = require_agent_identity(ctx, params.agent_id)
            except MCPAuthError as e:
                return f"Error: {e}"
