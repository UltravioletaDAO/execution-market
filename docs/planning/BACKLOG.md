...
|------|------|---------|----------|--------|
| 2026-06-12 | UI legible para el bloque de verificación del árbitro (Ring 1/Ring 2) en ReviewSubmission | Tras rediseñar EvidenceView (zero-JSON), la sección de veredicto del árbitro sigue mostrando jerga técnica cruda: "score: 88%, decision: pass, confidence: 90%", "CHEAP: PHOTINT score 0.88 >= pass threshold 0.75", "Arbiter Approved T0", "Costo: $0.000". Componentes: `EvidenceVerificationPanel.tsx`, `ArbiterVerdictBadge.tsx`, `EvidenceVerification.tsx`. Hacerlo legible y consistente con el resto (cápsulas/etiquetas en español, ocultar jerga T0/PHOTINT o traducir). Origen: feedback Saul Image #6 "casi perfecto pero faltó algo". | P2 | pending |
| 2026-06-12 | em_balances Lambda: RPC de celo (rpc.celocolombia.org) hace timeout 8s | Cada carga de balance no-cacheada tarda ~8s esperando el timeout de celo (las llamadas son concurrentes pero `as_completed` espera todas). Ralentiza el balance del worker en /profile. Fix: cambiar `public_rpc` de celo a uno confiable (ej. https://forno.celo.org) y/o verificar el private RPC de celo en `facilitator-rpc-mainnet`. Descubierto diagnosticando un "$0.00" falso en el panel de un worker (el balance real sí estaba, en Avalanche). | P2 | pending |
| 2026-06-12 | Activar ClawRouter + EigenAI como providers reales de Ring 2 (consenso dual) | Decisión Saul 2026-06-12 en el rings audit: son reales pero sin credenciales en ningún lado → por ahora single-provider OpenRouter documentado honesto (Task 2.4 de `MASTER_PLAN_RINGS_VERIFICATION_FIXES_2026-06-11.md`). Para activar: conseguir llaves, crear secrets `em/clawrouter` + `em/eigenai`, wiring ya listo (`providers/__init__.py` hace fallback limpio). | P2 | pending |
| 2026-06-11 | Recuperar $0.05 USDC lockeados huérfanos en escrow Base (TX 0x845176e7906c9b24254677e42b72a88cfbe7ea06fbc51f609cabe6d6a20ff5e2) | Run ref 24e7114a del e2e H2H: el Facilitator lockeó on-chain pero el relay crasheó parseando la respuesta (bug arreglado en 8f73fdf7) y el server hizo rollback sin persistir payment_info. **RESUELTO 2026-06-11**: paymentInfo decodificado del calldata de la TX → refund gasless vía Facilitator (action=refundInEscrow, sin private key) con `scripts/recover_orphan_escrow.py`. Refund TX 0x6ebde320a23722b357d188c669dae280723c7070a1490eb761c64789970716fd (block 47222478): $0.05 de vuelta al payer 0x857f...43Cd, escrow drenado (capturableAmount 0). Nota: el deposit autorizado vive en `capturableAmount`, NO en `refundableAmount` (eso solo cuenta post-capture). | P2 | resolved |
...
| 2026-03-26 | H2H "Trustless Rappi" — human hires human | Formalize H2H pipeline. Study Rappi/Uber Eats/DoorDash ops model. **Master plan creado 2026-06-04**: `MASTER_PLAN_HUMAN_HIRES_HUMAN_RAPPI.md` (7 fases, onramp MoonPay→USDC Base + catálogo de servicios web→móvil). En ejecución (goal "todas las fases"). | P1 | in-progress |
| 2026-06-04 | SECURITY: taximetro endpoints sin auth + IDOR | `/api/v1/taximetro/{channel_id}/{stream,history}` no autentican al caller ni verifican ownership del channel. Router flagged OFF (404) → NO explotable hoy. **Bloqueante antes de `EM_PAYSHELL_ENABLED=true`**: añadir auth ERC-8128 + ownership check via `task_channel_bindings`. Origen: security review commit 52876ab9. | P1 | pending |
| 2026-06-04 | SECURITY: payshell headers spoofables | `request.state.payshell` se arma de headers `x-payshell-*` client-settable sin verificar. Spoofables si el backend es alcanzable sin pasar por pay.sh. Flagged OFF → no usado hoy. **Bloqueante antes de activar flag**: strip de headers en ALB/edge para tráfico no-sidecar y/o HMAC firmado por pay.sh verificado en middleware. Docstring ya marca UNTRUSTED. Origen: security review commit 52876ab9. | P1 | pending |
