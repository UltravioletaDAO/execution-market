...
- `mcp_server/city_ops/coordination.py`, `dispatch_guidance.py`, `reuse.py`, `observability.py`, `closure.py`, `proof_block_artifacts.py`, `session_rebuild_consumer.py`, `acontext_transport.py`, `acontext_live_preflight.py`, `operator_debug_surface.py`, `proof_observability.py`, `coordination_intelligence.py`, `persisted_artifact_guardrail.py`, `proof_block_readiness.py`, `phase1_review_output_schemas.py`, and `phase1_review_normalizer.py` now carry the first anchor through continuity, runtime guidance, reuse, telemetry, bounded preview, persisted proof-block fixtures, one read-only rebuild consumer, one read-only rebuild report/debug fixture, Acontext transport parity, live preflight, thin operator/debug inspection, proof observability metrics, one read-only pattern-recognition packet for coordination strategy, a persisted-artifact guardrail, a readiness summary that reports whether artifacts plus local prerequisites are sufficient to attempt live Acontext transport, deterministic Phase 1 reviewed-output schema drafts, and a first review normalizer draft for the three Phase 1 offer cards.
...
- `PYTHONPATH=. python3 -m pytest mcp_server/tests/city_ops -q` passes with 125 tests and should stay as the city-ops gate before the next proof-fixture or Acontext-sink seam starts.
...
- `CITY_AS_A_SERVICE_PHASE_1_COUNTER_REALITY_FIXTURE_IMPLEMENTATION.md` adds the first concrete reviewed Counter Reality Check fixture: a stale/contradictory guidance case normalized into a reviewed redirect result with documented/observed/staff-heard source separation.
...
- Immediate next build: create the first Posting Compliance Check reviewed fixture, then a non-redirect Packet Submission Attempt fixture, then a tiny reviewed-fixture registry/summary for operator observability. Once Docker/local Acontext prerequisites are available, run one live write/retrieve transport pass from the same consumer bundle/report fields without semantic strengthening.
