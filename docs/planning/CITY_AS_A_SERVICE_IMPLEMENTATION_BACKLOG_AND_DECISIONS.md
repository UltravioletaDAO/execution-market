...
- Immediate next build: feed the normalizer with the first Counter Reality Check reviewed-output fixture, then the first Posting Compliance Check fixture, before adding a non-redirect Packet Submission Attempt fixture. Once Docker/local Acontext prerequisites are available, run one live write/retrieve transport pass from the same consumer bundle/report fields without semantic strengthening.
