...
> Last updated: 2026-05-07 07:00 America/New_York
> Parent docs:
...
- `mcp_server/city_ops/coordination.py`, `dispatch_guidance.py`, `reuse.py`, `observability.py`, `closure.py`, `proof_block_artifacts.py`, `session_rebuild_consumer.py`, `acontext_transport.py`, `acontext_live_preflight.py`, `operator_debug_surface.py`, `proof_observability.py`, `coordination_intelligence.py`, and `persisted_artifact_guardrail.py` now carry the first anchor through continuity, runtime guidance, reuse, telemetry, bounded preview, persisted proof-block fixtures, one read-only rebuild consumer, one read-only rebuild report/debug fixture, Acontext transport parity, live preflight, thin operator/debug inspection, proof observability metrics, one read-only pattern-recognition packet for coordination strategy, and a persisted-artifact guardrail that fails on dropped blocked claims, readiness overclaim, raw-source dependency, or worker-copyability drift.
...
- `PYTHONPATH=. python3 -m pytest mcp_server/tests/city_ops -q` passes with 85 tests and should stay as the PR #108 gate before the next Acontext-sink seam starts.
...
- `CITY_AS_A_SERVICE_ACONTEXT_TRANSPORT_PARITY_IMPLEMENTATION.md`, `CITY_AS_A_SERVICE_ACONTEXT_LIVE_PREFLIGHT_IMPLEMENTATION.md`, `CITY_AS_A_SERVICE_OPERATOR_DEBUG_SURFACE_IMPLEMENTATION.md`, `CITY_AS_A_SERVICE_PROOF_OBSERVABILITY_IMPLEMENTATION.md`, `CITY_AS_A_SERVICE_COORDINATION_INTELLIGENCE_IMPLEMENTATION.md`, and `CITY_AS_A_SERVICE_PERSISTED_ARTIFACT_GUARDRAIL_IMPLEMENTATION.md` are the May 7 transport/inspection/metrics/coordination-intelligence/guardrail notes.
