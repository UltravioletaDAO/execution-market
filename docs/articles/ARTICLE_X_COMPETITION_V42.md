...
> V42 - Apertura anclada en la realidad (demanda validada), OpenClaw reescrito, robots arriba
...
La primera semana de febrero de 2026, una plataforma donde agentes de IA contratan humanos generó **medio millón de visitas en un solo día**. Más de 73,000 personas se registraron para trabajar para máquinas — en 48 horas.

La demanda quedó demostrada. La tesis ya no es teoría.

Pero de esas 73,000 personas, casi ninguna completó una tarea. Sin escrow, no había garantía de pago. Sin reputación, no había confianza. Sin verificación, no había forma de probar que el trabajo se hizo.

La tarea emblemática — recoger un paquete por $40 — tuvo 30 aplicantes y **cero completaciones** en dos días.

**La demanda existe. La infraestructura no.**

Nosotros llevamos semanas construyendo esa infraestructura. Antes de que la demanda explotara. Escrow con reembolsos automáticos. Reputación on-chain portable. Verificación multi-nivel. Bounties dinámicos. Micro-tareas desde $0.25. Y algo que nadie más está haciendo: **soporte para robots desde el día uno.**

Se llama **Execution Market** — una **Universal Execution Layer**. No solo para humanos. Para todo lo que pueda ejecutar una tarea en el mundo físico.
...
**We're still learning in public.** Esto es lo que hemos estado construyendo.

---

## Por qué la demanda sin infraestructura no funciona

Lo que pasó esta semana es un caso de estudio perfecto de por qué este mercado necesita infraestructura real:

**Sin escrow**, el ejecutor no tiene garantía de cobrar. El agente no tiene garantía de que el trabajo se complete. Resultado: 30 personas aplican, nadie ejecuta, nadie cobra.

**Sin reputación**, el agente no puede distinguir entre un ejecutor confiable y un bot que se registró hace 5 minutos. Resultado: no hay confianza para asignar tareas.

**Sin verificación**, no hay forma de probar que el trabajo se hizo. ¿Cómo sabe el agente que la foto es real y no generada por IA? ¿Que el paquete fue entregado? Resultado: el agente no puede aprobar pagos con confianza.

**Sin soporte para robots**, estás construyendo para un mundo que va a cambiar en 2-3 años. Los robots humanoides llegan en 2026. Si tu plataforma solo soporta humanos, vas a tener que reconstruir todo.

Cada una de estas piezas — escrow, reputación, verificación, soporte universal — es lo que hemos estado construyendo. No como respuesta a lo que pasó esta semana. **Antes** de que pasara.
...
La demanda está probada. Medio millón de visitas en un día lo confirman. Pero hay una pregunta más profunda: **¿cómo conectas a millones de agentes de IA con la infraestructura de ejecución?**

La respuesta: **MCP** — Model Context Protocol.

MCP es el estándar que permite a los agentes de IA descubrir y usar herramientas externas. Piensa en MCP como el USB de los agentes — cualquier agente compatible puede conectarse a cualquier herramienta compatible. Plug and play.

Execution Market expone sus herramientas vía MCP:

```
em_publish_task    → Publicar una tarea
em_get_tasks       → Buscar tareas disponibles
em_check_submission → Verificar estado de un trabajo
em_approve_submission → Aprobar y pagar
```

Cualquier agente que hable MCP puede contratar ejecutores. Sin integración custom. Sin SDK propietario. Sin pedir permiso.
...
Millones de personas están empezando a usar agentes así. Y cada uno de esos agentes, eventualmente, va a necesitar algo del mundo físico.

"Necesito que alguien verifique si la tienda de la esquina ya abrió."
"Necesito que alguien lleve este paquete a la paquetería."
"Necesito que alguien tome una foto de esta dirección."

Hoy, esos agentes se quedan trabados. No tienen cuerpo.

**Con Execution Market, cualquier agente MCP-compatible tiene acceso instantáneo a un pool global de ejecutores — humanos y robots — con escrow, reputación, y verificación incluidos.**

### La distribución: Agentes como canal

No vamos a los usuarios finales. **Vamos a los agentes.**

Cada plataforma de agentes — OpenClaw, Claude, GPT, agentes custom, agentes enterprise — es un canal de distribución. Y MCP es el conector universal.
...
- **Coding assistants** que necesitan verificar deploys físicos
- **Agentes de investigación** que necesitan datos del mundo real

**Cada agente de IA es un cliente potencial de Execution Market.**

No competimos con los agentes. **Los habilitamos.** Y lo hacemos con un estándar abierto, no con un SDK cerrado.
...
MTurk, TaskRabbit, Fiverr — fueron diseñados para humanos contratando humanos. Las nuevas plataformas de "AI hires humans" mejoran la idea, pero siguen siendo **human-only**. 73,000 personas se registraron esta semana en una de ellas. Cero tareas completadas. Sin escrow. Sin reputación. Sin verificación. Sin soporte para robots.

**Execution Market es Universal Execution Layer.** Humanos Y robots desde el día uno. Escrow con reembolsos automáticos. Reputación on-chain portable. Verificación multi-nivel. Bounties dinámicos. Micro-tareas desde $0.25. Streaming de pagos.
...
### Llamadas y voz humana

Lo que muchos no consideran. Hay cosas que requieren una voz real al otro lado de la línea.

| Tarea | Pago |
|-------|------|
| Llamar a un negocio y confirmar horarios | $1-3 |
| Esperar en hold 45 minutos y transmitir info | $3-10 |
| Cancelar una suscripción que exige llamar | $3-10 |
| Llamar a oficina de gobierno por información específica | $5-15 |
| Negociar un precio o tarifa por teléfono | $5-20 |
| Hacer una reservación en restaurante sin booking online | $2-5 |

El agente puede enviar emails. Puede llenar formularios. Pero hay servicios que **te obligan a llamar**. Seguros, gobierno, cancelaciones. 45 minutos en hold. El agente no puede esperar en la línea. Tú sí. **$10.**

### Infraestructura y hardware

Esto es para los que manejan servidores y nodos. Cuando la administración remota falla, alguien tiene que ir.

| Tarea | Pago |
|-------|------|
| Verificar que un servidor/nodo está encendido | $5-15 |
| Presionar botón de reset físico en hardware | $10-20 |
| Fotografiar rack de servidores / cableado | $5-15 |
| Intercambiar USB / insertar llave de seguridad | $15-30 |
| Llevar/recoger hardware a un datacenter | $20-60 |

### Monitoreo y tareas recurrentes

No todo es una tarea puntual. A veces el agente necesita ojos persistentes.

| Tarea | Pago |
|-------|------|
| Foto diaria de un sitio de construcción | $2-5/día |
| Revisar un apartado postal y escanear contenido | $5-10/semana |
| Caminar una ruta y reportar condiciones | $5-15 |
| Monitorear una fila física y reportar tiempos | $3-10 |

### Lo que parece absurdo pero va a pasar

Tareas que suenan ridículas hasta que entiendes por qué un agente las necesita.
...
| Caminar por un barrio y describir la vibra | Agente inmobiliario, evaluación de comunidad | $5-15 |
| Contar objetos físicamente | Agente de inventario cuando cámara/scanner falla | $5-15 |

Un agente puede procesar millones de datos sensoriales digitales. Pero no puede saber si ese queso huele rancio. No puede sentir si esa tela es áspera. No puede describir si un barrio se siente seguro de noche.
...
```mermaid
graph LR
    A[Agente IA] -->|publica tarea| B[Execution Market Protocol]
    B -->|x402 pago| C[Worker]
    C -->|completa + evidencia| B
    B -->|verifica| D{¿Válido?}
    D -->|Sí| E[Pago instantáneo]
    D -->|No| F[x402r Reembolso]
    E --> G[ERC-8004 +rep]
    F --> H[ERC-8004 -rep]
```
-->
...
```mermaid
graph TD
    subgraph "HOY (horas/días)"
        A1[Agente cierra venta] --> B1[Notifica humano]
        B1 --> C1[Humano busca quien envíe]
        C1 --> D1[Coordina horarios/pago]
        D1 --> E1[Alguien va y envía]
        E1 --> F1[Reporta a humano]
        F1 --> G1[Humano actualiza agente]
    end

    subgraph "CON EXECUTION MARKET (minutos)"
        A2[Agente publica tarea $8] --> B2[Worker cercano la toma]
        B2 --> C2[Envía + foto recibo]
        C2 --> D2[Verificación automática]
        D2 --> E2[Pago instantáneo]
    end
```
-->
...
El agente no tiene que saber qué tipo de ejecutor es mejor. Execution Market tiene ambos en el pool. Publica la tarea, el mejor ejecutor disponible la toma.
...
**Execution Market ya tiene dashboard y MCP server desplegados:**
- Dashboard: [execution.market](https://execution.market)
- MCP Server: [mcp.execution.market](https://mcp.execution.market)
- X: [@executi0nmarket](https://x.com/executi0nmarket)
...
73,000 personas se registraron esta semana para trabajar para máquinas. La demanda es real. Lo que falta es infraestructura que funcione.

Queremos proveer esa infraestructura. Los rieles ya están. Ahora los estamos conectando.

Pero no pretendemos tener todas las respuestas. Hay preguntas incómodas que todavía no sabemos responder. Hay casos de uso que no hemos considerado. Hay objeciones válidas que probablemente no hemos escuchado.

**Por eso este artículo es una invitación — y un call to action.**

Si sos un **agente de IA** (o construís agentes): conectate vía MCP. Publicá tu primera tarea. Los tools están live en [mcp.execution.market](https://mcp.execution.market).

Si sos un **humano que quiere ejecutar tareas**: registrate en [execution.market](https://execution.market). Las primeras tareas están llegando.

Si tenés un **robot, dron, o hardware autónomo**: estamos diseñando la integración. Hablemos.

Si querés **ayudar a definir el protocolo**: las tecnologías base están (x402, x402r, Superfluid, ERC-8004), pero el estándar abierto todavía se está diseñando. Si tenés ideas sobre cómo debería funcionar, queremos escucharlas.
...
**La demanda explotó esta semana. La infraestructura ya existe. Ahora hay que conectarlas.**
...
*Execution Market ([@executi0nmarket](https://x.com/executi0nmarket)) es un proyecto de @UltravioletaDAO. Universal Execution Layer — humanos y robots, desde el día uno. Los rieles existen. Ahora construimos el puente.*
...
| V36 | 2026-02-03 | Narrativa de convergencia, tabla comparativa, URLs de producción, building in public |
...
| V41 | 2026-02-05 | Removida referencia a número de versiones del artículo ("Más de 35 versiones..."). Agregada cuenta de X @executi0nmarket en: URLs de progreso (apertura), sección "Quiénes somos", y línea de cierre |
| V42 | 2026-02-05 | **Apertura anclada en la realidad**: 500K visitas, 73K signups, 0 tareas completadas — la demanda existe, la infraestructura no. Sin nombrar competidor. Nueva sección "Por qué la demanda sin infraestructura no funciona" (escrow/reputación/verificación/robots). **OpenClaw reescrito**: ya no "el agente que usará EM" sino ejemplo de la ola de agentes personales; MCP como conector universal en vez de relación exclusiva. **Call-to-action concreto** en cierre: conectá tu agente, registrate como ejecutor, hablemos si tenés robots. Referencia a OpenClaw en sección robots eliminada |
