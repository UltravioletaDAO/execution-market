...
> V12 - Intro aprobada + estilo V6 + todos los detalles
...
No pueden verificar si un paquete llegó. No pueden hacer una llamada telefónica. No pueden completar un CAPTCHA. No pueden ir a notarizar un contrato.
...
El agente puede escribir código perfecto. Pero no puede verificar que la página se ve bien en tu teléfono Android de 2019. **$0.25.**
...
- Cada uno chocando contra CAPTCHAs, llamadas, verificaciones
...
El protocolo x402 permite pagar por request. Literalmente. HTTP + pago integrado. Sin cuentas. Sin intermediarios.

El equipo de x402r construyó refunds automáticos encima. Si el trabajo no se verifica, el dinero vuelve solo.
...
Perfecto para tareas complejas con múltiples pasos. Un agente abre canal, el humano ejecuta 5 verificaciones, al final se cierra todo. Sin pagar fees por cada micro-interacción.

### Streaming de pagos

El dinero fluye por segundo.

Imagina: un humano monitorea una ubicación por 2 horas. Su cámara transmite. El agente verifica en tiempo real. El dinero fluye mientras el trabajo se hace.
...
ERC-8004. Este es el componente crítico.

Cada participante - humano, agente, o eventualmente robot - tiene identidad verificable y portable.
...
No hay un solo juez. Cuatro niveles de verificación:

1. **Auto-check** (80%): GPS + timestamp + OCR. Si todo cuadra, pago automático.
2. **AI Review** (15%): Un modelo analiza la evidencia. Segundos.
3. **Agent Review** (4%): El agente que publicó revisa manualmente.
4. **Human Arbitration** (1%): Panel de 3 árbitros. Consenso 2-de-3.
...
1. Agente publica: "Enviar paquete a DHL, $8"
...
**El agente tiene cuerpo físico.** Ojos que observan. Manos que manipulan. Pies que se mueven. Dedos que completan CAPTCHAs. Voz que hace llamadas. A través de humanos que puede contratar on-demand.
...
Apps especializadas para nichos específicos (verificaciones inmobiliarias, CAPTCHAs en masa, auditorías de retail).
...
Una empresa de logística con agentes que publican tareas de verificación, y empleados que las toman como parte de su trabajo. Gamificado: streaks de 7 días = 1.5x puntos. Leaderboard mensual. El que más tareas completa gana reconocimiento.
...
Hay una ventana de 12-18 meses antes de que los robots domésticos sean viables a escala. La infraestructura funciona para ambos.
...
Los agentes de IA crecen exponencialmente. Cada día más capaces. Cada día chocan más fuerte contra el muro del mundo físico. Cada día chocan más fuerte contra CAPTCHAs y llamadas y verificaciones que no pueden hacer.
...
| V10 | 2026-01-21 | Título hook #8, enfoque transformacional |
| V11 | 2026-01-21 | Estilo V6 + tareas remotas/digitales + $0.25 mínimo |
| V12 | 2026-01-22 | Nuevo título + intro con ejemplos concretos + DHL |
