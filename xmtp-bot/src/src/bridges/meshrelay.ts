...
async function handleIrcMessage(channel: string, nick: string, text: string): Promise<void> {
  ...
  if (trimmed === "/help") {
    sendToChannel(channel, [
      ...
      "  /claim <task_id> — Apply to a task (requires wallet link)",
      "  /status <task_id> — Check task status",
      "  /link <wallet_address> — Link your IRC nick to a wallet",
      "  /help — Show this help message",
      "You can also link via: /msg MRServ LINK <wallet_address>",
    ].join("\n"));
    return;
  }
}
...
async function handleClaimCommand(channel: string, nick: string, text: string): Promise<void> {
  ...
  // Check identity mapping via MeshRelay API
  const walletAddress = await getWalletByNick(nick);
  if (!walletAddress) {
...
async function handleLinkCommand(channel: string, nick: string, text: string): Promise<void> {
  ...
  const result = await linkNickToWallet(nick, address);
  if (result.ok) {
    const short = `${address.slice(0, 6)}...${address.slice(-4)}`;
    sendToChannel(channel, `${nick}: Wallet linked: ${short}. You can now use /claim.`);
  } else {
    sendToChannel(channel, `${nick}: Link failed: ${result.error}. You can also link via /msg MRServ LINK <wallet>`);
  }
}
